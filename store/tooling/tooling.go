	...
	for i, comp := range comps {
		...
		if baseName == "" {
			cpi := snap.MinimalComponentContainerPlaceInfo(
				srr.Name, snap.R(srr.Revision), sar.SnapName().TODOSnapName())
		...
		downloadedComp, err := tsto.componentDownload(targetFn, sar.SnapName().TODOSnapName(), srr, snapOpts)
		if err != nil {
			return nil, err
		}
...
func (tsto *ToolingStore) snapDownload(targetFn string, sar *store.SnapActionResult, opts DownloadSnapOptions) (downloadedSnap *DownloadedSnap, err error) {
	...
	download := func(pb progress.Meter) error {
		...
		return tsto.sto.Download(context.TODO(), snap.SnapName().TODOSnapName(), targetFn,
...
func (tsto *ToolingStore) DownloadMany(toDownload []SnapToDownload, curSnaps []*CurrentSnap, opts DownloadManyOptions) (downloadedSnaps map[string]*DownloadedSnap, err error) {
	...
	for _, sn := range toDownload {
		actions = append(actions, &store.SnapAction{
			Action:         "download",
			InstanceName:   sn.Snap.SnapName().TODOSnapName(), // XXX consider using snap-id first
			Channel:        sn.Channel,
		...
		toDownloadByName[sn.Snap.SnapName().TODOSnapName()] = sn
	}
	...
	for _, sar := range sars {
		snapToDownload, ok := toDownloadByName[sar.SnapName().TODOSnapName()]
		if !ok {
		...
		for _, res := range sar.Resources {
			...
			cref := naming.NewComponentRef(sar.SnapName().TODOSnapName(), res.Name)
		...
		downloadedSnaps[sar.SnapName().TODOSnapName()] = dlSnap
		...
		downloadedSnaps[sar.SnapName().TODOSnapName()].Components = downloadedSnapComps
	}
