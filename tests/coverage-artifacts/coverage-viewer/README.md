...
```bash
go run ./tests/coverage-artifacts/coverage-viewer --results-dir ~/Desktop/spike/coverage-results/ -functions-json -test 'garden:ubuntu-24.04-64:tests--main--ack' | jq '.files |= map(select(any(.covered_functions[]; (. != "init") and (endswith(".Name") | not))))'
```
