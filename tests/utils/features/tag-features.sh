...
if [ -z "${FEATURES:-}" ]; then
	echo "since FEATURES was not set, tagging all features by default"
	export FEATURES="cmd,task,change,ensure,endpoint,interface"
fi
...
for i in $(seq 1 "$NUM_ATTEMPTS"); do
	...
	# Remove the last newline in the file if it exists
	if [ -s "${WRITE_DIR}/failed-tests.txt" ] && [ "$(tail -c1 "${WRITE_DIR}/failed-tests.txt")" == "" ]; then
		truncate -s -1 "${WRITE_DIR}/failed-tests.txt"
	fi
...
if [ -s "${WRITE_DIR}"/featextract.error ]; then
	echo "Encountered some errors during feature extraction. See ${WRITE_DIR}/featextract.error"
fi
