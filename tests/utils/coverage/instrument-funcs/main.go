...
func main() {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "main")
	root := "."
	if len(os.Args) > 1 {
		root = os.Args[1]
	}
...
func instrumentFile(path, wd string) error {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "instrumentFile")
...
func strLit(s string) string {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "strLit")
	return fmt.Sprintf("%q", s)
}

func hasLoggerImport(f *ast.File) bool {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "hasLoggerImport")
...
func isAlreadyInstrumented(body *ast.BlockStmt) bool {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "isAlreadyInstrumented")
	if len(body.List) == 0 {
		return false
	}
...
func addImportToSource(src []byte, fset *token.FileSet, f *ast.File, importPath string) []byte {
	logger.Trace("coverage", "file", "tools/instrument-funcs/main.go", "func", "addImportToSource")
	importLine := "\n\t" + importPath
