...
func applicable(s snap.PlaceInfo, t snap.Type, dev snap.Device) bool {
	...
	case snap.TypeKernel:
		if s.InstanceName().TODOInstanceName() != dev.Kernel() {
	...
	case snap.TypeBase, snap.TypeOS:
		...
		if s.InstanceName().TODOInstanceName() != base {
			return false
		}
	case snap.TypeGadget:
		...
		if !dev.HasModeenv() || s.InstanceName().TODOInstanceName() != dev.Gadget() {
			return false
		}
	default:
		return false
...
func InUse(typ snap.Type, dev snap.Device) (InUseFunc, error) {
	...
	return func(name string, rev snap.Revision) bool {
		for _, cand := range cands {
			if cand.SnapName().TODOSnapName() == name && cand.SnapRevision() == rev {
				return true
			}
		}
		return false
