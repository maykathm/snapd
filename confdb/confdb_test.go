...
func (s *viewSuite) TestMissingRequestDefaultsToStorage(c *C) {
	...
	err = view.Set(databag, "a.b", "value", []confdb.Visibility{})
	c.Assert(err, IsNil)

	value, err := view.Get(databag, "", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestGetAndSetViews(c *C) {
	...
	// nested string value
	err = view.Set(databag, "ssid", "my-ssid", []confdb.Visibility{})
	c.Assert(err, IsNil)

	ssid, err := view.Get(databag, "ssid", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Check(ssid, DeepEquals, "my-ssid")

	// nested list value
	err = view.Set(databag, "ssids", []string{"one", "two"}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	ssids, err := view.Get(databag, "ssids", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Check(ssids, DeepEquals, []any{"one", "two"})

	// top-level string
	err = view.Set(databag, "top-level", "randomValue", []confdb.Visibility{})
	c.Assert(err, IsNil)

	topLevel, err := view.Get(databag, "top-level", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Check(topLevel, DeepEquals, "randomValue")

	// dotted request paths are permitted
	err = view.Set(databag, "dotted.path", 3, []confdb.Visibility{})
	c.Assert(err, IsNil)

	num, err := view.Get(databag, "dotted.path", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestSetWithNilValueFail(c *C) {
	...
	err = view.Set(databag, "foo", "value", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, ErrorMatches, `internal error: Set value cannot be nil`)

	ssid, err := view.Get(databag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestConfdbNotFoundErrors(c *C) {
	...
	_, err = view.Get(databag, "missing", nil, []confdb.Visibility{})
	...
	err = view.Set(databag, "missing", "thing", []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoMatchError{})
	...
	_, err = view.Get(databag, "top-level", nil, []confdb.Visibility{})
	...
	_, err = view.Get(databag, "", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	c.Assert(err, ErrorMatches, `cannot get acc/foo/bar: no data`)

	err = view.Set(databag, "nested", "thing", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Unset(databag, "nested")
	c.Assert(err, IsNil)

	_, err = view.Get(databag, "other-nested", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
...
func (s *viewSuite) TestConfdbNoMatchAllSubkeyTypes(c *C) {
	...
	for _, request := range []string{"b", "a[1]", "a.b.c", "a.b[1].d"} {
		_, err = view.Get(databag, request, nil, []confdb.Visibility{})
		c.Assert(err, testutil.ErrorIs, &confdb.NoMatchError{})
	...
	_, err = view.Get(databag, "a.b[1][0]", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (s *viewSuite) TestViewBadRead(c *C) {
	...
	view := schema.View("bar")
	err = view.Set(databag, "one", "foo", []confdb.Visibility{})
	c.Assert(err, IsNil)

	_, err = view.Get(databag, "onetwo", nil, []confdb.Visibility{})
...
func (s *viewSuite) TestViewAccessControl(c *C) {
	for _, t := range []struct {
		...
		err = view.Set(databag, "foo", "thing", []confdb.Visibility{})
		if t.setErr != "" {
		...
		_, err = view.Get(databag, "foo", nil, []confdb.Visibility{})
		if t.getErr != "" {
...
func (s *viewSuite) TestViewAssertionWithPlaceholder(c *C) {
	for _, t := range []struct {
		...
		err = view.Set(databag, t.request, "expectedValue", []confdb.Visibility{})
		c.Assert(err, IsNil, cmt)

		value, err := view.Get(databag, t.request, nil, []confdb.Visibility{})
		c.Assert(err, IsNil, cmt)
...
func (s *viewSuite) TestViewUnsetTopLevelEntry(c *C) {
	...
	view := schema.View("my-view")
	err = view.Set(databag, "foo", "fval", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "bar", "bval", []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	value, err := view.Get(databag, "bar", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestViewUnsetLeafWithSiblings(c *C) {
	...
	view := schema.View("my-view")
	err = view.Set(databag, "bar", "barVal", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "baz", "bazVal", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Unset(databag, "bar")
	c.Assert(err, IsNil)

	_, err = view.Get(databag, "bar", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})

	// doesn't affect the other leaf entry under "foo"
	value, err := view.Get(databag, "baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestViewUnsetWithNestedEntry(c *C) {
	...
	val, err := view.Get(databag, "foo", nil, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})

	val, err = view.Get(databag, "bar", nil, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (s *viewSuite) TestViewUnsetLeafDoesntLeaveEmptyParentMap(c *C) {
	...
	err = view.Set(databag, "foo.bar", "val", []confdb.Visibility{})
	c.Assert(err, IsNil)

	value, err := view.Get(databag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(databag, "a", map[string]any{"c": "d"}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Unset(databag, "a.c")
	c.Assert(err, IsNil)

	value, err = view.Get(databag, "a.c", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	c.Assert(value, IsNil)
}
...
func (s *viewSuite) TestViewUnsetLeafDoesntLeaveEmptyParentList(c *C) {
	...
	err = view.Set(bag, "b", []any{map[string]any{"c": "d"}, map[string]any{"c": "e"}}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	val, err := view.Get(bag, "b[1]", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	c.Assert(val, IsNil)
	...
	val, err = view.Get(bag, "b", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	c.Assert(val, IsNil)
}
...
func (s *viewSuite) TestViewGetResultNamespaceMatchesRequest(c *C) {
	...
	value, err := view.Get(databag, "one.two", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(value, DeepEquals, "value")

	value, err = view.Get(databag, "onetwo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	// the key matches the request, not the storage storage
	c.Assert(value, DeepEquals, "value")

	value, err = view.Get(databag, "one", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestViewGetMatchesOnPrefix(c *C) {
	...
	value, err := view.Get(databag, "snapd.status", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(value, DeepEquals, "active")

	value, err = view.Get(databag, "snapd", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestViewGetNoMatchRequestLongerThanPattern(c *C) {
	...
	_, err = view.Get(databag, "snapd.status", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoMatchError{})
}
...
func (s *viewSuite) TestViewManyPrefixMatches(c *C) {
	...
	view := schema.View("statuses")
	err = view.Set(databag, "status.firefox", "active", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "status.snapd", "disabled", []confdb.Visibility{})
	c.Assert(err, IsNil)

	value, err := view.Get(databag, "status", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestGetScalarOverwritesLeafOfMapValue(c *C) {
	...
	value, err := view.Get(databag, "motors", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestGetUnmatchedPlaceholderReturnsAll(c *C) {
	...
	value, err := view.Get(databag, "snaps", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestGetSeveralUnmatchedPlaceholders(c *C) {
	...
	value, err := view.Get(databag, "a", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestBadRequestPaths(c *C) {
	...
	err = view.Set(databag, "", "value", []confdb.Visibility{})
	c.Assert(err, NotNil, cmt)
...
func (s *viewSuite) TestSetAllowedOnSameRequestButDifferentPaths(c *C) {
	...
	err = view.Set(databag, "a.b.c", "value", []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestReadWriteRead(c *C) {
	...
	data, err := view.Get(databag, "a", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(data, DeepEquals, initData)

	err = view.Set(databag, "a", data, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestReadWriteSameDataAtDifferentLevels(c *C) {
	...
	for _, req := range []string{"a", "a.b", "a.b.c"} {
		val, err := view.Get(databag, req, nil, []confdb.Visibility{})
		c.Assert(err, IsNil)

		err = view.Set(databag, req, val, []confdb.Visibility{})
		c.Assert(err, IsNil)
	}
...
func (s *viewSuite) TestSetValueMissingNestedLevels(c *C) {
	...
	err = view.Set(databag, "a", "foo", []confdb.Visibility{})
	...
	err = view.Set(databag, "a", map[string]any{"c": "foo"}, []confdb.Visibility{})
	...
	err = view.Set(databag, "b", "foo", []confdb.Visibility{})
...
func (s *viewSuite) TestGetReadsStorageLessNestedNamespaceBefore(c *C) {
	...
	data, err := view.Get(databag, "snaps", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *viewSuite) TestSetValidateError(c *C) {
	...
	err = view.Set(databag, "bar", "baz", []confdb.Visibility{})
...
func (s *viewSuite) TestSetOverwriteValueWithNewLevel(c *C) {
	...
	bag := confdb.NewJSONDatabag()
	err = view.Set(bag, "a", "foo", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(bag, "c", []any{"bar"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(bag, "a.b", "foo", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(bag, "c[0][0]", "bar", []confdb.Visibility{})
	c.Assert(err, IsNil)

	data, err := view.Get(bag, "", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(data, DeepEquals, map[string]any{
		"a": map[string]any{
			"b": "foo",
...
func (s *viewSuite) TestSetValidatesDataWithSchemaPass(c *C) {
	...
	err = view.Set(databag, "foo", map[string]int{"a": 1, "b": 2}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "bar", []string{"one", "two"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
}
...
func (s *viewSuite) TestSetPreCheckValueAllowsIntNumberMismatch(c *C) {
	...
	err = view.Set(databag, "foo", 1, []confdb.Visibility{})
	c.Assert(err, IsNil)

	// the schema still checks the data at the end, so setting int schema to a float fails
	err = view.Set(databag, "foo", 1.1, []confdb.Visibility{})
...
func (*viewSuite) TestSchemaMismatchCheckMultipleAlternativeTypesHappy(c *C) {
	...
	err = view.Set(databag, "foo", true, []confdb.Visibility{})
	c.Assert(err, IsNil)
}
...
func (s *viewSuite) TestSetUnmatchedPlaceholderLeaf(c *C) {
	...
	data, err := view.Get(databag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(data, DeepEquals, map[string]any{
		"bar": "value",
		"baz": "other",
...
func (s *viewSuite) TestUnsetUnmatchedPlaceholderMid(c *C) {
	...
	val, err := view.Get(databag, "all", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"a": map[string]any{
			"two": "other",
...
func (s *viewSuite) TestViewSetErrorIfValueContainsUnusedParts(c *C) {
	...
	for i, tc := range tcs {
		...
		err = view.Set(databag, tc.request, tc.value, []confdb.Visibility{})
		if tc.err != "" {
...
func (s *viewSuite) TestGetEntireView(c *C) {
	...
	err = view.Set(databag, "abc", "cba", []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(databag, "write-only", "value", []confdb.Visibility{})
	c.Assert(err, IsNil)

	result, err := view.Get(databag, "", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestViewContentRule(c *C) {
	...
	view := schema.View("bar")
	val, err := view.Get(databag, "a.b", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "value")

	err = view.Set(databag, "a.b", "other", []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestContentInheritsAccess(c *C) {
	for _, t := range []struct {
		...
		view := schema.View("foo")
		err = view.Set(databag, "foo.bar", "thing", []confdb.Visibility{})
		if t.setErr != "" {
			c.Assert(err, NotNil)
			c.Assert(err.Error(), Equals, t.setErr, cmt)
		} else {
		...
		_, err = view.Get(databag, "foo.bar", nil, []confdb.Visibility{})
		if t.getErr != "" {
			c.Assert(err, NotNil)
			c.Assert(err.Error(), Equals, t.getErr, cmt)
		} else {
...
func (*viewSuite) TestInheritedAccessInSeveralNestedContents(c *C) {
	...
	view := schema.View("foo")
	val, err := view.Get(databag, "foo.bar.baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "abc")

	err = view.Set(databag, "foo.bar.baz", "abc", []confdb.Visibility{})
...
func (*viewSuite) TestViewSeveralNestedContentRules(c *C) {
	...
	databag := confdb.NewJSONDatabag()
	view := schema.View("bar")
	err = view.Set(databag, "a.b.c.d", "value", []confdb.Visibility{})
	c.Assert(err, IsNil)

	val, err := view.Get(databag, "a.b.c.d", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "value")
}
...
func (*viewSuite) TestSetValidateValue(c *C) {
	...
	for _, tc := range tcs {
		...
		err = view.Set(databag, "foo", tc.value, []confdb.Visibility{})
		if tc.error != "" {
			c.Assert(err, NotNil, cmt)
...
func (*viewSuite) TestGetListLiteral(c *C) {
	...
	view := schema.View("foo")
	val, err := view.Get(bag, "a[0].bar", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, float64(1337))

	val, err = view.Get(bag, "a[1].baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, float64(999))

	val, err = view.Get(bag, "top", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	val, err = view.Get(bag, "nested", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestGetListPlaceholder(c *C) {
	...
	val, err = view.Get(bag, "b[1][0].baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, float64(999))

	val, err = view.Get(bag, "a", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	val, err = view.Get(bag, "c", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestGetListPlaceholderValueNotFound(c *C) {
	...
	view := schema.View("foo")
	_, err = view.Get(bag, "c", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})

	// path goes beyond stored list
	_, err = view.Get(bag, "c[2]", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (*viewSuite) TestDetectViewRulesExpectDifferentTypes(c *C) {
	...
	_, err = view.Get(bag, "a.b", nil, []confdb.Visibility{})
...
func (*viewSuite) TestSetListSetsOrAppends(c *C) {
	...
	val, err := view.Get(bag, "a[0]", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, "foo")

	// can overwrite
	err = view.Set(bag, "a[0]", "bar", []confdb.Visibility{})
	c.Assert(err, IsNil)
	val, err = view.Get(bag, "a[0]", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	// can append
	err = view.Set(bag, "a[1]", "baz", []confdb.Visibility{})
	c.Assert(err, IsNil)
	val, err = view.Get(bag, "a[1]", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, "baz")

	// cannot append if index is not next one
	err = view.Set(bag, "a[9]", "foo", []confdb.Visibility{})
...
func (*viewSuite) TestSetListNested(c *C) {
	...
	// set nested list
	err = view.Set(bag, "a[0][0]", "foo", []confdb.Visibility{})
	c.Assert(err, IsNil)

	// append in nested list
	err = view.Set(bag, "a[0][1]", "bar", []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestSetListPlaceholder(c *C) {
	...
	// can set entire list
	err = view.Set(bag, "a", []any{"foo"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(bag, "a", []any{"foo", "bar"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(bag, "a[0]", map[string]any{"a": "b"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(bag, "a[1]", map[string]any{"c": "d"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestListMerge(c *C) {
	...
	err = view.Set(bag, "a[2]", "baz", []confdb.Visibility{})
	c.Assert(err, IsNil)

	res, err := view.Get(bag, "a", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestUnsetList(c *C) {
	...
	err = view.Set(bag, "a", []any{"foo", "bar", "baz"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	err = view.Set(bag, "b", []any{map[string]any{"c": "foo"}, map[string]any{"c": "bar"}}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Unset(bag, "b")
	c.Assert(err, IsNil)

	_, err = view.Get(bag, "b", nil, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (*viewSuite) TestUnsetBeyondCurrentList(c *C) {
	...
	for _, pref := range []string{"a", "c"} {
		err = view.Set(bag, pref, []any{map[string]any{"b": "foo"}, map[string]any{"b": "bar"}}, []confdb.Visibility{})
		c.Assert(err, IsNil)

		err = view.Unset(bag, pref+"[2]")
		c.Assert(err, IsNil)

		val, err := view.Get(bag, pref, nil, []confdb.Visibility{})
		c.Assert(err, IsNil)
...
func (*viewSuite) TestPartialUnsetNestedInList(c *C) {
	...
	err = view.Set(bag, "a", []any{map[string]any{"a": "foo"}, map[string]any{"a": "foo"}}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Set(bag, "b", []any{map[string]any{"b": "bar"}, map[string]any{"b": "bar"}}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	err = view.Unset(bag, "a")
	c.Assert(err, IsNil)

	val, err := view.Get(bag, "", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
...
func (*viewSuite) TestUnsetNestedList(c *C) {
	...
	err = view.Set(bag, "a", []any{[]any{"foo", "bar", "baz"}, []any{"a", "b"}}, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestFieldFilteringBasic(c *C) {
	...
	err = bag.Set(parsePath(c, "snaps"), map[string]any{
		"snap-a": map[string]any{
			"status":  "disabled",
			"version": "1.0",
		},
	...
	view := schema.View("foo")
	val, err := view.Get(bag, "", map[string]any{"status": "disabled"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"snaps": map[string]any{
			"snap-a": map[string]any{
	...
	val, err = view.Get(bag, "snaps", map[string]any{"status": "disabled", "version": "2"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"snap-b": map[string]any{
			"status":  "disabled",
...
func (*viewSuite) TestFieldFilteringManyLevels(c *C) {
	...
	bag := confdb.NewJSONDatabag()
	err = bag.Set(parsePath(c, "alice"), map[string]any{
		"age": "21",
	...
	err = bag.Set(parsePath(c, "bob"), map[string]any{
		"age": "30",
		"odie": map[string]any{
			"toy":  "ball",
			"kind": "dog",
	...
	err = bag.Set(parsePath(c, "charlie"), map[string]any{
		"age": "21",
		"scruffles": map[string]any{
			"toy":  "bone",
			"kind": "dog",
		},
	})
	...
	view := schema.View("foo")
	val, err := view.Get(bag, "users", map[string]any{"age": "21"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	...
	val, err = view.Get(bag, "pet-kinds", map[string]any{"toy": "ball"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals,
		map[string]any{
			"alice": map[string]any{
...
func (*viewSuite) TestFilteringNoData(c *C) {
	...
	view := schema.View("foo")
	_, err = view.Get(bag, "", map[string]any{"field": "baz"}, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	...
	val, err := view.Get(bag, "foo", map[string]any{"field": "not-there"}, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})

	err = view.Set(bag, "a", map[string]any{"b": map[string]any{"c": map[string]any{"d": "value"}}}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	// pruning several nested levels down
	val, err = view.Get(bag, "a", map[string]any{"field": "not-there"}, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (*viewSuite) TestFieldFilteringNestedUnderList(c *C) {
	...
	err = view.Set(bag, "foo", []any{
		map[string]any{"baz": "a"},
		map[string]any{"baz": "b"},
	}, []confdb.Visibility{})
	c.Assert(err, IsNil)

	val, err := view.Get(bag, "foo", map[string]any{"field": "a"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (*viewSuite) TestListFiltering(c *C) {
	...
	view := schema.View("foo")
	val, err := view.Get(bag, "vm", map[string]any{"path": "dirty_bytes"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals,
		[]any{
			map[string]any{
	...
	val, err = view.Get(bag, "vm-value", map[string]any{"path": "dirty_bytes"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, []any{"10"})

	val, err = view.Get(bag, "vm[0]", map[string]any{"path": "not-there"}, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})

	val, err = view.Get(bag, "vm", map[string]any{"path": "not-there"}, []confdb.Visibility{})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (*viewSuite) TestFieldFilteringNotString(c *C) {
	...
	bag := confdb.NewJSONDatabag()
	view := schema.View("foo")
	err = view.Set(bag, "foo.bar", true, []confdb.Visibility{})
	c.Assert(err, IsNil)

	// can't unmarshal the boolean as string so considered as not matching
	val, err := view.Get(bag, "foo", map[string]any{"bar": "true"}, []confdb.Visibility{})
	c.Check(err, testutil.ErrorIs, &confdb.NoDataError{})
	c.Assert(val, IsNil)
}
...
func (*viewSuite) TestTypedFilteringConstraints(c *C) {
	...
	for i, tc := range tcs {
		cmt := Commentf("testcase %d/%d", i+1, len(tcs))
		val, err := view.Get(bag, "bar", tc.constraint, []confdb.Visibility{})
...
func (s *viewSuite) TestConfdbGetFilterCheck(c *C) {
	...
	_, err = view.Get(bag, "foo", map[string]any{"a": "a", "b": "b"}, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (s *viewSuite) TestFilteringSubkey(c *C) {
	...
	val, err := view.Get(bag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"a": map[string]any{"baz": "a"},
		"b": map[string]any{"baz": "b"},
	...
	val, err = view.Get(bag, "foo", map[string]any{"bar": "b"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"b": map[string]any{"baz": "b"},
	})

	val, err = view.Get(bag, "foo", map[string]any{"bar": "a"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"a": map[string]any{"baz": "a"},
	})

	_, err = view.Get(bag, "foo", map[string]any{"bar": "c"}, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}
...
func (s *viewSuite) TestFilterNestedSubkey(c *C) {
	...
	val, err := view.Get(bag, "foo", map[string]any{"baz": "c"}, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{
		"a": map[string]any{
			"c": "1",
	...
	_, err = view.Get(bag, "foo", map[string]any{"baz": "e"}, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
	...
	_, err = view.Get(bag, "foo", map[string]any{"baz": "c", "bar": "b"}, []confdb.Visibility{})
	c.Assert(err, testutil.ErrorIs, &confdb.NoDataError{})
}

func (s *viewSuite) TestGetSecretData(c *C) {
	views := map[string]any{
	...
	val, err := view.Get(bag, "foo.bar", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "secret")

	val, err = view.Get(bag, "foo.bar", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(val, IsNil)
	...
	val, err = view.Get(bag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{"bar": "secret"})

	val, err = view.Get(bag, "foo", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(err, testutil.ErrorIs, &confdb.UnAuthorizedAccessError{})
	c.Assert(err.Error(), Equals, `cannot get "foo" through acc/foo/foo: unauthorized access`)
	c.Assert(val, IsNil)
}

func (s *viewSuite) TestGetSecretDataDiffStoragePath(c *C) {
	views := map[string]any{
		"foo": map[string]any{"rules": []any{
	...
	val, err := view.Get(bag, "baz.eph", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "secret")

	val, err = view.Get(bag, "baz.eph", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.UnAuthorizedAccessError{})
	c.Assert(err.Error(), Equals, `cannot get "baz.eph" through acc/foo/foo: unauthorized access`)

	val, err = view.Get(bag, "baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{"eph": "secret"})

	val, err = view.Get(bag, "baz", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(err, testutil.ErrorIs, &confdb.UnAuthorizedAccessError{})
	c.Assert(err, ErrorMatches, `cannot get "baz" through acc/foo/foo: unauthorized access`)
	c.Assert(val, IsNil)
}

func (s *viewSuite) TestGetDataFromSecretPath(c *C) {
	views := map[string]any{
		"foo": map[string]any{"rules": []any{
			map[string]any{"request": "foo.{a}", "storage": "foo.{a}"},
			map[string]any{"request": "foo.bar.baz", "storage": "foo.bar.baz"},
	...
	view := schema.View("foo")
	err = bag.Set(parsePath(c, "foo"), map[string]any{
		"bar": map[string]any{"baz": "b"},
		"eph": "e"},
	)
	c.Assert(err, IsNil)

	val, err := view.Get(bag, "foo.bar.baz", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, Equals, "b")

	val, err = view.Get(bag, "foo.bar.baz", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(val, IsNil)
	c.Assert(err, testutil.ErrorIs, &confdb.UnAuthorizedAccessError{})
	c.Assert(err.Error(), Equals, `cannot get "foo.bar.baz" through acc/foo/foo: unauthorized access`)

	val, err = view.Get(bag, "foo", nil, []confdb.Visibility{})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{"bar": map[string]any{"baz": "b"}, "eph": "e"})

	val, err = view.Get(bag, "foo", nil, []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(err, IsNil)
	c.Assert(val, DeepEquals, map[string]any{"eph": "e"})
}

func (s *viewSuite) TestSetSecretData(c *C) {
	views := map[string]any{
		"foo": map[string]any{"rules": []any{
			map[string]any{"request": "foo.bar", "storage": "foo.bar"},
		}},
	...
	err = view.Set(bag, "foo.bar", "baz", []confdb.Visibility{confdb.SecretVisibility})
...
func (s *viewSuite) TestSetSecretDataDiffStoragePath(c *C) {
	views := map[string]any{
		"foo": map[string]any{"rules": []any{
			map[string]any{"request": "baz.eph", "storage": "foo.bar"},
		}},
	...
	err = view.Set(bag, "baz.eph", "secret", []confdb.Visibility{confdb.SecretVisibility})
	_, ok := err.(*confdb.UnAuthorizedAccessError)
	c.Assert(ok, Equals, true)
...
func (s *viewSuite) TestSetSecretPathData(c *C) {
	views := map[string]any{
		"foo": map[string]any{"rules": []any{
			map[string]any{"request": "foo.{a}", "storage": "foo.{a}"},
			map[string]any{"request": "foo.bar.baz", "storage": "foo.bar.baz"},
	...
	err = view.Set(bag, "foo.bar.baz", "b", []confdb.Visibility{confdb.SecretVisibility})
	c.Assert(err, testutil.ErrorIs, &confdb.UnAuthorizedAccessError{})
