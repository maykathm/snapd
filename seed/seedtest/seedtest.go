...
func (ss *SeedSnaps) makeAssertedSnap(
	...
	snapID := ss.AssertedSnapID(snapName.TODOSnapName())
	headers := map[string]any{
		"series":       "16",
		"snap-id":      snapID,
		"publisher-id": developerID,
		"snap-name":    snapName.TODOSnapName(),
	...
	ss.snaps[snapName.TODOSnapName()] = snapFile
	info.SideInfo.RealName = snapName.TODOSnapName()
	ss.infos[snapName.TODOSnapName()] = info
	snapDecl := declA.(*asserts.SnapDeclaration)
	snapRev := revA.(*asserts.SnapRevision)
	ss.snapRevs[snapName.TODOSnapName()] = snapRev
	...
	for _, comp := range info.Components {
		cref := naming.NewComponentRef(snapName.TODOSnapName(), comp.Name)
	...
	ss.compInfos[snapName.TODOSnapName()] = cinfos
	ss.comps[snapName.TODOSnapName()] = resResults
	ss.resRevs[snapName.TODOSnapName()] = resRevs
	ss.resPairs[snapName.TODOSnapName()] = resPairs
...
func (s *TestingSeed20) MakeSeedWithModel(c *C, label string, model *asserts.Model, optSnaps []*seedwriter.OptionsSnap, compPathsBySnap map[string][]string) {
	...
	for _, sn := range localSnaps {
		...
		compPaths := compPathsBySnap[info.SnapName().TODOSnapName()]
	...
	fetchAsserts := func(sn, _, _ *seedwriter.SeedSnap) ([]*asserts.Ref, error) {
		...
		prev := len(sf.Refs())
		if err = sf.Save(s.snapRevs[sn.SnapName().TODOSnapName()]); err != nil {
			return nil, err
		}
		...
		for _, resRev := range s.resRevs[sn.SnapName().TODOSnapName()] {
		...
		for _, resPair := range s.resPairs[sn.SnapName().TODOSnapName()] {
	...
	for {
		...
		for _, sn := range snaps {
			...
			info := s.AssertedSnapInfo(name.TODOSnapName())
			...
			cinfos := s.AssertedComponentInfos(name.TODOSnapName())
			...
			err = os.Rename(s.AssertedSnap(name.TODOSnapName()), sn.Path)
			c.Assert(err, IsNil)
			for _, comp := range sn.Components {
