...
func (s *seed20Suite) TestLoadMetaCore20Iter(c *C) {
	...
	err = seed20.Iter(func(sn *seed.Snap) error {
		seen[sn.SnapName().TODOSnapName()] = true
		return nil
	})
	c.Assert(err, IsNil)
	...
	err = seed20.Iter(func(sn *seed.Snap) error {
		if sn.SnapName() == "core20" {
			return fmt.Errorf("mock error for snap %q", sn.SnapName().TODOSnapName())
		}
		return nil
	})
...
func (s *seed20Suite) TestLoadMetaWithComponents(c *C) {
	...
	c.Check(runSnaps, HasLen, 1)
	req20sn := runSnaps[0]
	c.Check(req20sn.SnapName().TODOSnapName(), Equals, "required20")
