...
func (w *Writer) modelSnapToSeed(modSnap *asserts.ModelSnap) (*SeedSnap, error) {
	...
	channel, err := w.resolveChannel(modSnap.SnapName().TODOSnapName(), modSnap, optSnap)
	if err != nil {
		return nil, err
	}
...
func (w *Writer) extraSnapToSeed(optSnap *OptionsSnap) (*SeedSnap, error) {
	...
	channel, err := w.resolveChannel(sn.SnapName().TODOSnapName(), nil, optSnap)
	if err != nil {
		return nil, err
	}
	sn.Channel = channel
...
func (w *Writer) installedSnaps() []*snapasserts.InstalledSnap {
	installedSnap := func(snap *SeedSnap) *snapasserts.InstalledSnap {
		return snapasserts.NewInstalledSnap(snap.SnapName().TODOSnapName(), snap.ID(), snap.Info.Revision, nil)
	}
	...
	seedSnaps := func(snaps []*SeedSnap) error {
		for _, sn := range snaps {
			...
			if !sn.local {
				...
				if err := copySnap(info.SnapName().TODOSnapName(), sn.Path, dst); err != nil {
					return err
				}
				// copy components
			...
			if !info.Revision.Unset() {
				if err := w.manifest.MarkSnapRevisionSeeded(sn.Info.SnapName().TODOSnapName(), sn.Info.Revision); err != nil {
