...
func (s *writerSuite) fetchAsserts(c *C) seedwriter.AssertsFetchFunc {
	return func(sn, sysSn, kSn *seedwriter.SeedSnap) ([]*asserts.Ref, error) {
		...
		if sysSn == nil {
			...
			c.Check(sysSn.SnapName().TODOSnapName(), Equals, s.expectedSysSnap)
			c.Check(sysSn.Path, Not(Equals), "")
		}
		if kSn == nil {
			c.Assert(s.expectedKernSnap, Equals, "", Commentf("no kernel should be expected"))
		} else {
			c.Check(kSn.SnapName().TODOSnapName(), Equals, s.expectedKernSnap)
			c.Check(kSn.Path, Not(Equals), "")
		}
		aRefs := s.aRefs[sn.SnapName().TODOSnapName()]
		if aRefs == nil {
			prev := len(s.rf.Refs())
			err := s.rf.Fetch(s.AssertedSnapRevision(sn.SnapName().TODOSnapName()).Ref())
			if err != nil {
				return nil, err
			}
			for _, a := range s.AssertedResourceRevision(sn.SnapName().TODOSnapName()) {
				err := s.rf.Fetch(a.Ref())
				if err != nil {
					return nil, err
				}
			}
			for _, a := range s.AssertedResourcePair(sn.SnapName().TODOSnapName()) {
				err := s.rf.Fetch(a.Ref())
				if err != nil {
					return nil, err
				}
			...
			s.aRefs[sn.SnapName().TODOSnapName()] = aRefs
		}
		return aRefs, nil
	}
}
...
func (s *writerSuite) doFillMetaDownloadedSnap(c *C, w *seedwriter.Writer, sn *seedwriter.SeedSnap) *snap.Info {
	info := s.AssertedSnapInfo(sn.SnapName().TODOSnapName())
	cinfos := s.AssertedComponentInfos(sn.SnapName().TODOSnapName())
	seedComps := make(map[string]*seedwriter.SeedComponent, len(cinfos))
	for _, cinfo := range cinfos {
		cref := naming.NewComponentRef(sn.SnapName().TODOSnapName(), cinfo.Component.ComponentName)
		seedComps[cinfo.Component.ComponentName] = &seedwriter.SeedComponent{
			ComponentRef: cref,
			Info:         cinfo,
		}
	}
	c.Assert(info, NotNil, Commentf("%s not defined", sn.SnapName().TODOSnapName()))
...
func (s *writerSuite) fillDownloadedSnap(c *C, w *seedwriter.Writer, sn *seedwriter.SeedSnap) {
	...
	err := os.Rename(s.AssertedSnap(sn.SnapName().TODOSnapName()), sn.Path)
	c.Assert(err, IsNil)
...
func (s *writerSuite) TestSeedSnapsWriteMetaCore16(c *C) {
	...
	for i, name := range []string{"core", "pc-kernel", "pc"} {
		...
		c.Check(seedYaml.Snaps[i], DeepEquals, &seedwriter.InternalSnap16{
			Name:    info.SnapName().TODOSnapName(),
			SnapID:  info.SnapID,
			Channel: channel,
			File:    fn,
...
func (s *writerSuite) TestSetRedirectChannelErrors(c *C) {
	...
	sn := snaps[4]
	c.Assert(sn.SnapName().TODOSnapName(), Equals, "required18")
...
func (s *writerSuite) TestCore20NonDangerousDisallowedDevmodeSnaps(c *C) {
	...
	c.Assert(snaps[4].SnapName().TODOSnapName(), Equals, "my-devmode")
	sn := snaps[4]
...
func (s *writerSuite) TestSnapsToDownloadCore20OptionalSnaps(c *C) {
	...
	c.Assert(err, IsNil)
	c.Check(snaps, HasLen, 6)
	c.Check(snaps[5].SnapName().TODOSnapName(), Equals, "optional20-b")
}
...
func (s *writerSuite) TestSeedSnapsWriteMetaCore20ExtraSnaps(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(snaps, HasLen, 3)
	c.Check(snaps[0].SnapName().TODOSnapName(), Equals, "cont-producer")
	c.Check(snaps[1].SnapName().TODOSnapName(), Equals, "core18")
	c.Check(snaps[2].SnapName().TODOSnapName(), Equals, "required20")
