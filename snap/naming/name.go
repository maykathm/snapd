...
// ParseSnapName validates s as a snap name and returns it as a SnapName.
func ParseSnapName(s string) (SnapName, error) {
	if err := ValidateSnap(s); err != nil {
		return "", err
	}
	return SnapName(s), nil
}

// ParseInstanceName validates s as a snap instance name and returns it as an
// InstanceName.
func ParseInstanceName(s string) (InstanceName, error) {
	if err := ValidateInstance(s); err != nil {
		return "", err
	}
	return InstanceName(s), nil
}
