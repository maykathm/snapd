...
func mockSnap(c *check.C, instanceName, yamlText string, sideInfo *snap.SideInfo) *snap.Info {
	...
	if instanceName != "" {
		...
		c.Assert(snapInfo.InstanceName().TODOInstanceName(), check.Equals, instanceName)
		c.Assert(snapInfo.SnapName().TODOSnapName(), check.Equals, snapName)
	}
...
func MockComponent(c *check.C, yamlText string, info *snap.Info, csi snap.ComponentSideInfo) *snap.ComponentInfo {
	...
	mountDir := snap.ComponentMountDir(infoForName.Component.ComponentName, csi.Revision, info.InstanceName().TODOInstanceName())
...
func MockComponentCurrent(c *check.C, yamlText string, info *snap.Info, csi snap.ComponentSideInfo) *snap.ComponentInfo {
	...
	mountDir := snap.ComponentMountDir(ci.Component.ComponentName, ci.Revision, info.InstanceName().TODOInstanceName())
	link := filepath.Join(snap.ComponentsBaseDir(info.InstanceName().TODOInstanceName()), info.Revision.String(), ci.Component.ComponentName)
