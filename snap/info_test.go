...
func (s *infoSuite) TestSideInfoOverrides(c *C) {
	...
	c.Check(info.InstanceName().TODOInstanceName(), Equals, "newname")
...
func (s *infoSuite) TestReadInfo(c *C) {
	...
	c.Check(snapInfo2.InstanceName().TODOInstanceName(), Equals, "sample")
...
func (s *infoSuite) TestReadInfoWithInstance(c *C) {
	...
	c.Check(snapInfo2.InstanceName().TODOInstanceName(), Equals, "sample_instance")
	c.Check(snapInfo2.SnapName().TODOSnapName(), Equals, "sample")
	c.Check(snapInfo2.Revision, Equals, snap.R(42))
...
func (s *infoSuite) TestInstallDate(c *C) {
	...
	c.Check(snap.InstallDate(info.InstanceName().TODOInstanceName()).IsZero(), Equals, true)
	...
	c.Check(snap.InstallDate(info.InstanceName().TODOInstanceName()).Equal(instTime), Equals, true)
}
...
func (s *infoSuite) TestReadInfoDanglingSymlink(c *C) {
	...
	info, err := snap.ReadInfo("sample", si)
	c.Check(err, IsNil)
	c.Check(info.SnapName().TODOSnapName(), Equals, "test")
...
confinement: devmode`
	...
	c.Assert(err, IsNil)
	c.Check(info.InstanceName().TODOInstanceName(), Equals, "foo")
...
type: app`
	...
	c.Assert(err, IsNil)
	c.Check(info.InstanceName().TODOInstanceName(), Equals, "baz")
	c.Check(info.Version, Equals, "1.0")
	c.Check(info.Type(), Equals, snap.TypeApp)
	c.Check(info.Revision, Equals, snap.R(42))
}
...
func (s *infoSuite) checkInstalledSnapAndSnapFile(c *C, instanceName, yaml string, contents string, hooks []string, checker func(c *C, info *snap.Info)) {
	...
	info, err := snap.ReadInfo(info0.InstanceName().TODOInstanceName(), sideInfo)
	c.Check(err, IsNil)
	checker(c, info)
...
func (s *infoSuite) TestParsePlaceInfoFromSnapFileName(c *C) {
	...
	for _, t := range tt {
		...
		if t.expectErr != "" {
			...
			c.Check(p.SnapName().TODOSnapName(), Equals, t.name)
...
func (s *infoSuite) TestAppDesktopFile(c *C) {
	...
	c.Check(snapInfo.InstanceName().TODOInstanceName(), Equals, "sample")
	...
	c.Check(snapInfo.InstanceName().TODOInstanceName(), Equals, "sample_instance")
...
func (s *infoSuite) TestInstanceNameInSnapInfo(c *C) {
	...
	c.Check(info.InstanceName().TODOInstanceName(), Equals, "snap-name_foo")
	c.Check(info.SnapName().TODOSnapName(), Equals, "snap-name")

	info.InstanceKey = ""
	c.Check(info.InstanceName().TODOInstanceName(), Equals, "snap-name")
	c.Check(info.SnapName().TODOSnapName(), Equals, "snap-name")
}
