...
func Validate(info *Info) error {
	...
	if err := ValidateName(info.SnapName().TODOSnapName()); err != nil {
		return err
	}
	if err := ValidateInstanceName(name.TODOInstanceName()); err != nil {
		return err
	}
...
func ValidateLayoutAll(info *Info) error {
	...
	thisSnapMntDir := filepath.Join("/snap/", info.SnapName().TODOSnapName())
	for _, path := range paths {
...
func (prqt *SelfContainedSetPrereqTracker) Check() (warnings, errs []error) {
	...
	for _, info := range prqt.snaps {
		...
		for _, plugName := range plugNames {
			...
			case 1:
				if candSlots[0].Snap.InstanceName().TODOInstanceName() == defaultProvider {
					continue
				}
