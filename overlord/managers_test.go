...
func (s *baseMgrsSuite) SetUpTest(c *C) {
	...
	r := snapshotstate.MockBackendSave(func(_ context.Context, id uint64, si *snap.Info, cfg map[string]any, usernames []string,
		...
		s.automaticSnapshots = append(s.automaticSnapshots, automaticSnapshotCall{InstanceName: si.InstanceName().TODOInstanceName(), SnapConfig: cfg, Usernames: usernames, Options: options})
		return nil, nil
	})
	s.AddCleanup(r)
...
func (ms *baseMgrsSuite) mockInstalledSnapWithRevAndFiles(c *C, snapYaml string, rev snap.Revision, files [][]string) *snap.Info {
	...
	si := &snap.SideInfo{
		...
		SnapID:   fakeSnapID(info.SnapName().TODOSnapName()),
...
func (s *baseMgrsSuite) makeStoreTestSnapWithFiles(c *C, snapYaml string, revno string, files [][]string) (path, digest string) {
	...
	s.makeStoreSnapRevision(c, info.SnapName().TODOSnapName(), revno, snapDigest, size)
...
func (s *baseMgrsSuite) serveSnap(snapPath, revno string) {
	...
	name := info.SnapName()
	s.serveIDtoName[fakeSnapID(name.TODOSnapName())] = name.TODOSnapName()

	if oldPath := s.serveSnapPath[name.TODOSnapName()]; oldPath != "" {
		oldRev := s.serveRevision[name.TODOSnapName()]
		if oldRev == "" {
			panic("old path set but not old revision")
		}
		s.serveOldPaths[name.TODOSnapName()] = append(s.serveOldPaths[name.TODOSnapName()], oldPath)
		s.serveOldRevs[name.TODOSnapName()] = append(s.serveOldRevs[name.TODOSnapName()], oldRev)
	}
	s.serveSnapPath[name.TODOSnapName()] = snapPath
	s.serveRevision[name.TODOSnapName()] = revno
}
...
func (s *mgrsSuite) installLocalTestSnap(c *C, snapYamlContent string) *snap.Info {
	...
	var snapst snapstate.SnapState
	snapstate.Get(st, snapName.TODOInstanceName(), &snapst)

	ts, err := snapstate.InstallPath(st, &snap.SideInfo{RealName: snapName.TODOInstanceName()}, snapPath, "", "", snapstate.Flags{DevMode: true}, nil)
	c.Assert(err, IsNil)
	chg := st.NewChange("install-snap", "...")
	chg.AddAll(ts)
...
func verifyModelEssentialSnapHasContent(c *C, sd seed.Seed, name string, file, content string) {
	for _, ms := range sd.EssentialSnaps() {
		c.Logf("mode snap %q %v", ms.SnapName().TODOSnapName(), ms.Path)
		if ms.SnapName().TODOSnapName() == name {
	...
	autoConnectStatus := func(inWait string, done []string) {
		...
		for _, tsk := range chg.Tasks() {
			if tsk.Kind() == "auto-connect" {
				...
				snapsup, err := snapstate.TaskSnapSetup(tsk)
				c.Assert(err, IsNil)
				if snapsup.InstanceName().TODOInstanceName() == inWait {
					expectedStatus = state.DoStatus
				} else if strutil.ListContains(done, snapsup.InstanceName().TODOInstanceName()) {
					expectedStatus = state.DoneStatus
				}
...
func snapTaskStatusForChange(chg *state.Change) map[string]state.Status {
	...
	for _, t := range chg.Tasks() {
		if snapsup, err := snapstate.TaskSnapSetup(t); err == nil {
			taskStates[snapsup.SnapName().TODOSnapName()+":"+t.Kind()] = t.Status()
		}
	}
	return taskStates
}
			...
			SetupCallback: func(appSet *interfaces.SnapAppSet, copts interfaces.ConfinementOptions, sctx interfaces.SetupContext, repo *interfaces.Repository) error {
				...
				if ifacemgrReinitDone {
					...
					case strings.HasPrefix(name.String(), "consumer"):
						...
						return nil
					default:
						return fmt.Errorf("unexpected call for snap %q", appSet.InstanceName().TODOInstanceName())
					}
				}
				return nil
			},
