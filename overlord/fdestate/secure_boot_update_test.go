...
func (s *fdeMgrSuite) testEFISecurebootNoSealedKeysForKind(
	...
	err = fdestate.EFISecurebootDBUpdatePrepare(st, kind, [][]byte{[]byte("payload")})
	c.Assert(err, IsNil)
...
func (s *fdeMgrSuite) testEFISecurebootPrepareHappyForKind(
	...
	c.Check(fdeSt.PendingExternalOperations[0], DeepEquals, fdestate.ExternalOperation{
		...
		ChangeID: "1",
		Context: []byte(
			fmt.Sprintf(`{"payloads":["cGF5bG9hZA=="],"sealing-method":"tpm","db":%d}`, kind)),
...
func (s *fdeMgrSuite) testEFISecurebootPrepareConflictSelfForKind(
	...
	err = func() error {
		st.Unlock()
		defer st.Lock()
		return fdestate.EFISecurebootDBUpdatePrepare(st, kind, [][]byte{[]byte("payload")})
	}()
