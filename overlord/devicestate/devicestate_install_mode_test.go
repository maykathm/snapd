...
`, compData[0].name, compData[1].name)
	...
	for _, comp := range compData {
		...
		cpi := snap.MinimalComponentContainerPlaceInfo(comp.name, comp.rev, kernelInfo.SnapName().TODOSnapName())
...
func (s *deviceMgrInstallModeSuite) TestInstallCheckEncrypted(c *C) {
	...
	for _, tc := range []struct {
		...
		if !tc.hasTPM && !tc.hasFDESetupHook {
			c.Check(logbuf.String(), Matches, "[\\s\\S]*: not encrypting device storage as checking TPM gave: cannot connect to TPM device\n")
		}
		logbuf.Reset()
	}
}
