...
func (h *configureHandler) Before() error {
	...
	if useDefaults {
		...
		patch, err = snapstate.ConfigDefaults(st, deviceCtx, instanceName.TODOInstanceName())
		...
		if len(patch) != 0 && instanceName != naming.Core {
			...
			info, err := snapstate.CurrentInfo(st, instanceName.TODOInstanceName())
			if err != nil {
				return err
			}
	...
	if err := config.Patch(tr, instanceName.TODOInstanceName(), patch); err != nil {
		return err
	}
