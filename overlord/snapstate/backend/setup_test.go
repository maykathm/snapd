...
`, testFiles)
	...
	c.Assert(bloader.ExtractKernelAssetsCalls[0].InstanceName().TODOInstanceName(), Equals, "kernel")
	...
	c.Assert(bloader.RemoveKernelAssetsCalls[0].InstanceName().TODOInstanceName(), Equals, "kernel")
}
...
`, testFiles)
	...
	c.Assert(bloader.ExtractKernelAssetsCalls[1].InstanceName().TODOInstanceName(), Equals, "kernel")
	minInfo := snap.MinimalPlaceInfo("kernel", snap.R(140))
...
func (s *setupSuite) TestRemoveSnapFilesDir(c *C) {
	...
	c.Assert(osutil.FileExists(minInfo.MountDir()), Equals, false)
	c.Assert(osutil.FileExists(minInfo.MountFile()), Equals, false)
	c.Assert(osutil.FileExists(snap.BaseDir(minInfo.InstanceName().TODOInstanceName())), Equals, true)
	c.Assert(osutil.FileExists(snap.BaseDir(minInfo.SnapName().TODOSnapName())), Equals, true)

	// /snap/hello is kept as other instances exist
	err = s.be.RemoveSnapDir(minInfo, true)
	c.Assert(err, IsNil)
	c.Assert(osutil.FileExists(snap.BaseDir(minInfo.InstanceName().TODOInstanceName())), Equals, false)
	c.Assert(osutil.FileExists(snap.BaseDir(minInfo.SnapName().TODOSnapName())), Equals, true)

	// /snap/hello is removed when there are no more instances
	err = s.be.RemoveSnapDir(minInfo, false)
	c.Assert(err, IsNil)
	c.Assert(osutil.FileExists(snap.BaseDir(minInfo.SnapName().TODOSnapName())), Equals, false)
}
