...
func affectedByRefresh(st *state.State, updates []string) (map[string]*AffectedSnapInfo, error) {
	...
	if !release.OnClassic {
		...
		bootBase = bootBaseInfo.InstanceName().TODOInstanceName()
	}
	...
	for _, snapName := range updates {
		...
		if snapSt := snapsWithHook[up.InstanceName().TODOInstanceName()]; snapSt != nil {
			addAffected(up.InstanceName().TODOInstanceName(), up.InstanceName().TODOInstanceName(), false, false)
		}

		// on core system, affected by update of boot base
		if bootBase != "" && up.InstanceName().TODOInstanceName() == bootBase {
			for _, snapSt := range snapsWithHook {
				addAffected(snapSt.InstanceName().TODOInstanceName(), up.InstanceName().TODOInstanceName(), true, false)
			}
		}
		...
		if up.Type() == snap.TypeBase || up.SnapName().TODOSnapName() == "core" {
			// affected by refresh of this base snap
			for _, snapName := range byBase[up.InstanceName().TODOInstanceName()] {
				addAffected(snapName, up.InstanceName().TODOInstanceName(), false, true)
			}
		}
		...
		// by snap updates.
		if up.SnapType != snap.TypeSnapd && up.SnapName().TODOSnapName() != "core" {
			for _, slotInfo := range up.Slots {
				conns, err := repo.Connected(up.InstanceName().TODOInstanceName(), slotInfo.Name)
				if err != nil {
					return nil, err
				}
				for _, cref := range conns {
					// affected only if it wasn't optimized out above
					if snapsWithHook[cref.PlugRef.Snap.TODOInstanceName()] != nil {
						addAffected(cref.PlugRef.Snap.TODOInstanceName(), up.InstanceName().TODOInstanceName(), true, false)
					}
				}
			}
		}
...
var snapsToRefresh = func(gatingTask *state.Task) ([]*refreshCandidate, error) {
	...
	for _, s := range snaps {
		if _, ok := held[s.InstanceName().TODOInstanceName()]; !ok {
			candidates = append(candidates, s)
		} else {
			skipped = append(skipped, s.InstanceName().TODOInstanceName())
		}
	}
