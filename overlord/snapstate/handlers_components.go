...
func compSetupAndState(t *state.Task) (*ComponentSetup, *SnapSetup, *SnapState, error) {
	...
	err = Get(t.State(), ssup.InstanceName().TODOInstanceName(), &snapst)
	if err != nil && !errors.Is(err, state.ErrNoState) {
		return nil, nil, nil, err
	}
...
func (m *SnapManager) doDownloadComponent(t *state.Task, tomb *tomb.Tomb) error {
	...
	target := compsup.BlobPath(snapsup.InstanceName().TODOInstanceName())
...
func (m *SnapManager) doMountComponent(t *state.Task, _ *tomb.Tomb) (retErr error) {
	...
	cpi := snap.MinimalComponentContainerPlaceInfo(compSetup.ComponentName(),
		csi.Revision, snapsup.InstanceName().TODOInstanceName())
...
func ReadComponentInfo(snapInfo *snap.Info, csi *snap.ComponentSideInfo) (*snap.ComponentInfo, error) {
	...
	mountDir := snap.ComponentMountDir(compName, compRev, snapInfo.InstanceName().TODOInstanceName())
...
func (m *SnapManager) undoMountComponent(t *state.Task, _ *tomb.Tomb) error {
	...
	return m.undoSetupComponent(t, compSetup.CompSideInfo, snapsup.InstanceName().TODOInstanceName(),
...
func (m *SnapManager) doLinkComponent(t *state.Task, _ *tomb.Tomb) error {
	...
	csi := cs.SideInfo
	cpi := snap.MinimalComponentContainerPlaceInfo(csi.Component.ComponentName,
		csi.Revision, snapInfo.InstanceName().TODOInstanceName())
...
func (m *SnapManager) doUnlinkCurrentComponent(t *state.Task, _ *tomb.Tomb) (err error) {
	...
	if err := m.unlinkComponent(
		t, snapSt, snapInfo.InstanceName().TODOInstanceName(), snapInfo.Revision, cref); err != nil {
		return err
	}

	// Finally, write the state
	Set(st, snapInfo.InstanceName().TODOInstanceName(), snapSt)
	// Make sure we won't be rerun
	t.SetStatus(state.DoneStatus)
...
func (m *SnapManager) doUnlinkComponent(t *state.Task, _ *tomb.Tomb) (err error) {
	...
	if err := m.unlinkComponent(
		t, snapSt, snapSup.InstanceName().TODOInstanceName(), snapSup.Revision(), cref); err != nil {
		return err
	}
...
func (m *SnapManager) undoUnlinkCurrentComponent(t *state.Task, _ *tomb.Tomb) (err error) {
	...
	if err := m.relinkComponent(
		t, snapSt, snapInfo.InstanceName().TODOInstanceName(), snapInfo.Revision); err != nil {
		return err
	}
...
func (m *SnapManager) undoUnlinkComponent(t *state.Task, _ *tomb.Tomb) (err error) {
	...
	if err := m.relinkComponent(
		t, snapSt, snapSup.InstanceName().TODOInstanceName(), snapSup.Revision()); err != nil {
		return err
	}
...
func (m *SnapManager) doPrepareKernelModulesComponents(t *state.Task, _ *tomb.Tomb) error {
	...
	err = m.backend.SetupKernelModulesComponents(
		snapsup.PreUpdateKernelModuleComponents, newComps, snapsup.InstanceName().TODOInstanceName(), snapsup.Revision(), pm,
	)
	st.Lock()
	if err != nil {
		return err
...
func (m *SnapManager) undoPrepareKernelModulesComponents(t *state.Task, _ *tomb.Tomb) error {
	...
	pm := NewTaskProgressAdapterUnlocked(t)
	err = m.backend.SetupKernelModulesComponents(
		justSetupComps, snapsup.PreUpdateKernelModuleComponents, snapsup.InstanceName().TODOInstanceName(), snapsup.Revision(), pm,
	)
	st.Lock()
	if err != nil {
		return err
...
func infoForCompUndo(t *state.Task) (*sequence.ComponentState, string, error) {
	...
	return &unlinkedComp, snapsup.InstanceName().TODOInstanceName(), nil
}
