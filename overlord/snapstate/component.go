...
func newComponentInstallChoreographer(
	...
	if err := checkChangeConflictIgnoringOneChange(st, snapsup.InstanceName().TODOInstanceName(), snapst, copts); err != nil {
		return nil, err
	}
...
func (cc *componentInstallChoreographer) BeforeLink(st *state.State, s *taskChainSpan) ([]*state.Task, error) {
	...
	if !cc.snapsup.Revert && cc.installed() {
		preRefreshHook := SetupPreRefreshComponentHook(st, cc.snapsup.InstanceName().TODOInstanceName(), csi.Component.ComponentName)
...
func (cc *componentInstallChoreographer) PostHookToBeforeDiscard(st *state.State, s *taskChainSpan) ([]*state.Task, error) {
	...
	if !cc.installed() {
		hook := SetupInstallComponentHook(st, cc.snapsup.InstanceName().TODOInstanceName(), csi.Component.ComponentName)
		s.Append(hook)
	} else {
		hook := SetupPostRefreshComponentHook(st, cc.snapsup.InstanceName().TODOInstanceName(), csi.Component.ComponentName)
		s.Append(hook)
	}
