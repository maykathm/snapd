...
func mockInstalledSnap(c *C, st *state.State, snapYaml string, hasHook bool) *snap.Info {
	...
	si := &snap.SideInfo{RealName: snapName.TODOSnapName(), SnapID: snapName.TODOSnapName() + "-id", Revision: snap.R(1)}
	snapstate.Set(st, snapName.TODOSnapName(), &snapstate.SnapState{
		Active:   true,
...
func (s *autorefreshGatingSuite) TestAffectedByBase(c *C) {
	...
	updates := []string{baseSnapA.InstanceName().TODOInstanceName()}
...
func (s *autorefreshGatingSuite) TestAffectedByCore(c *C) {
	...
	updates := []string{core.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-c": {
...
func (s *autorefreshGatingSuite) TestAffectedByKernel(c *C) {
	...
	updates := []string{kernel.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-c": {
...
func (s *autorefreshGatingSuite) TestAffectedBySelf(c *C) {
	...
	snapC := mockInstalledSnap(c, s.state, snapCyaml, useHook)
	updates := []string{snapC.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-c": {
...
func (s *autorefreshGatingSuite) TestAffectedBySlot(c *C) {
	...
	cref := &interfaces.ConnRef{PlugRef: interfaces.PlugRef{Snap: naming.InstanceName("snap-e"), Name: "plug"}, SlotRef: interfaces.SlotRef{Snap: naming.InstanceName("snap-d"), Name: "slot"}}
	_, err = s.repo.Connect(cref, nil, nil, nil, nil, nil)
	c.Assert(err, IsNil)

	updates := []string{snapD.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-e": {
...
func (s *autorefreshGatingSuite) TestNotAffectedByCoreOrSnapdSlot(c *C) {
	...
	cref := &interfaces.ConnRef{PlugRef: interfaces.PlugRef{Snap: naming.InstanceName("snap-g"), Name: "mir"}, SlotRef: interfaces.SlotRef{Snap: naming.InstanceName("core"), Name: "mir"}}
	_, err = s.repo.Connect(cref, nil, nil, nil, nil, nil)
	c.Assert(err, IsNil)
...
func (s *autorefreshGatingSuite) TestNotAffectedByPlugWithMountBackend(c *C) {
	...
	updates := []string{snapE.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, HasLen, 0)
}
...
func (s *autorefreshGatingSuite) TestAffectedByPlugWithMountBackendSnapdSlot(c *C) {
	...
	c.Assert(s.repo.AddAppSet(snapGAppSet), IsNil)
	cref := &interfaces.ConnRef{PlugRef: interfaces.PlugRef{Snap: naming.InstanceName("snap-g"), Name: "desktop"}, SlotRef: interfaces.SlotRef{Snap: naming.InstanceName("snapd"), Name: "desktop"}}
	_, err = s.repo.Connect(cref, nil, nil, nil, nil, nil)
	c.Assert(err, IsNil)

	// snapE has a plug using mount backend, refreshing snapd affects snapE.
	updates := []string{snapdSnap.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-g": {
...
func (s *autorefreshGatingSuite) TestAffectedByPlugWithMountBackendCoreSlot(c *C) {
	...
	c.Assert(s.repo.AddAppSet(coreAppSet), IsNil)
	c.Assert(s.repo.AddAppSet(snapGAppSet), IsNil)
	cref := &interfaces.ConnRef{PlugRef: interfaces.PlugRef{Snap: naming.InstanceName("snap-g"), Name: "desktop"}, SlotRef: interfaces.SlotRef{Snap: naming.InstanceName("core"), Name: "desktop"}}
	_, err = s.repo.Connect(cref, nil, nil, nil, nil, nil)
	c.Assert(err, IsNil)

	// snapG has a plug using mount backend, refreshing core affects snapE.
	updates := []string{coreSnap.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-g": {
...
func (s *autorefreshGatingSuite) TestAffectedByBootBase(c *C) {
	...
	updates := []string{core18.InstanceName().TODOInstanceName()}
	affected, err := snapstate.AffectedByRefresh(st, updates)
	c.Assert(err, IsNil)
	c.Check(affected, DeepEquals, map[string]*snapstate.AffectedSnapInfo{
		"snap-a": {
...
func (s *snapmgrTestSuite) testAutoRefreshPhase2DiskSpaceCheck(c *C, fail bool) {
	...
	restoreInstallSize := snapstate.MockInstallSize(func(st *state.State, snaps []snapstate.MinimalInstallInfo, userID int, prqt snapstate.PrereqTracker) (uint64, error) {
		...
		for _, sn := range snaps {
			seen[sn.InstanceName().TODOInstanceName()] = true
		}
...
func (s *snapmgrTestSuite) TestAutoRefreshPhase2GatedSnaps(c *C) {
	...
	restore := snapstate.MockSnapsToRefresh(func(gatingTask *state.Task) ([]*snapstate.RefreshCandidate, error) {
		...
		for _, cand := range candidates {
			seenSnaps[cand.InstanceName().TODOInstanceName()] = true
			if cand.InstanceName().TODOInstanceName() == "snap-a" {
				continue
			}
