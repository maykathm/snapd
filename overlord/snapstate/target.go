...
func targetFromLocalSnapWithStoreComponents(
	...
	info, err := readInfo(snapst.InstanceName().TODOInstanceName(), si, errorOnBroken)
	if err != nil {
		return target{}, err
	}
...
func (s *storeInstallGoal) toInstall(ctx context.Context, st *state.State, opts Options) ([]target, error) {
	...
	for _, r := range results {
		sn, ok := s.snap(r.InstanceName().TODOInstanceName())
		if !ok {
			return nil, fmt.Errorf("store returned unsolicited snap action: %s", r.InstanceName())
		}

		snapst, ok := allSnaps[r.InstanceName().TODOInstanceName()]
		if !ok {
			snapst = &SnapState{}
		}
	...
	for _, t := range installs {
		sn, ok := s.snap(t.info.InstanceName().TODOInstanceName())
		if !ok {
...
func checkSnapAgainstValidationSets(info *snap.Info, components []ComponentSetup, action string, vsets *snapasserts.ValidationSets) error {
	...
	if err := checkSnapAgainstConstraints(info.InstanceName().TODOInstanceName(), info.Revision, constraints, action); err != nil {
		return err
	}
	...
	return checkComponentsAgainstConstraints(info.SnapName().TODOSnapName(), comps, constraints, action)
}
...
func componentSetupFromResource(name string, sar store.SnapResourceResult, info *snap.Info) (ComponentSetup, error) {
	...
	cref := naming.NewComponentRef(info.SnapName().TODOSnapName(), name)
...
func (p *updatePlan) revisionChanges(st *state.State, opts Options) ([]*snap.Info, error) {
	...
	for _, t := range p.targets {
		targetByName[t.info.InstanceName().TODOInstanceName()] = t
	}
	...
	for _, up := range updates {
		...
		t, ok := targetByName[up.SnapState.InstanceName().TODOInstanceName()]
...
func (p *updatePlan) filterHeldSnaps(st *state.State, opts Options) error {
	...
	p.filter(func(t target) (bool, error) {
		_, ok := heldSnaps[t.info.InstanceName().TODOInstanceName()]
		return !ok, nil
	})
...
func (p *updatePlan) validateAndFilterTargets(st *state.State, opts Options) error {
	...
	for _, t := range p.targets {
		...
		if t.snapst.IgnoreValidation || !t.snapst.IsInstalled() {
			ignoreValidation[t.info.InstanceName().TODOInstanceName()] = true
		}
	}
	...
	for _, sn := range validated {
		validatedMap[sn.InstanceName().TODOInstanceName()] = true
	}

	p.filter(func(t target) (bool, error) {
		_, ok := validatedMap[t.info.InstanceName().TODOInstanceName()]
		return ok, nil
	})
...
func initRefreshAllStoreUpdates(st *state.State, opts Options, allSnaps map[string]*SnapState) (map[string]StoreUpdate, error) {
	...
	for _, snapst := range allSnaps {
		updates[snapst.InstanceName().TODOInstanceName()] = StoreUpdate{
			InstanceName: snapst.InstanceName().TODOInstanceName(),
