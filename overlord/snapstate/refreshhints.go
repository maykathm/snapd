...
func refreshHintsFromUpdatePlan(st *state.State, plan updatePlan, deviceCtx DeviceContext) (map[string]*refreshCandidate, error) {
	...
	for _, t := range plan.targets {
		...
		if err := Get(st, info.InstanceName().TODOInstanceName(), &snapst); err != nil {
			return nil, err
		}
		...
		hints[info.InstanceName().TODOInstanceName()] = &refreshCandidate{
			SnapSetup:  snapsup,
			Components: compsups,
			Monitored:  IsSnapMonitored(st, info.InstanceName().TODOInstanceName()),
		}
	}
	return hints, nil
}
