...
func (f *fakeSnappyBackend) ClearTrashedData(si *snap.Info) {
	f.appendOp(&fakeOp{
		op:    "cleanup-trash",
		name:  si.InstanceName().TODOInstanceName(),
		revno: si.Revision,
	})
}
...
func (f *fakeSnappyBackend) MaybeSetNextBoot(
	...
	if f.linkSnapMaybeReboot {
		reboot = info.Type() == snap.TypeKernel || info.InstanceName().TODOInstanceName() == dev.Base() ||
			(f.linkSnapRebootFor != nil && f.linkSnapRebootFor[info.InstanceName().TODOInstanceName()])
	}
...
func (f *fakeSnappyBackend) UndoSetupSnap(s snap.PlaceInfo, typ snap.Type, installRecord *backend.InstallRecord, dev snap.Device, p progress.Meter) error {
	...
	f.appendOp(op)
	if s.InstanceName().TODOInstanceName() == "borken-undo-setup" {
...
func (f *fakeSnappyBackend) RemoveSnapSaveData(info *snap.Info, _ snap.Device) error {
	op := &fakeOp{
		...
		path: snap.CommonDataSaveDir(info.InstanceName().TODOInstanceName()),
	}
	f.appendOp(op)
	return f.maybeErr(op)
}
...
func (f *fakeSnappyBackend) RemoveSnapDataDir(info *snap.Info, otherInstances bool, opts *dirs.SnapDirOptions) error {
	op := &fakeOp{
		...
		name:           info.InstanceName().TODOInstanceName(),
		path:           snap.BaseDataDir(info.InstanceName().TODOInstanceName()),
...
func (f *fakeSnappyBackend) RemoveSnapDir(s snap.PlaceInfo, otherInstances bool) error {
	op := &fakeOp{
		op:             "remove-snap-dir",
		name:           s.InstanceName().TODOInstanceName(),
		path:           snap.BaseDir(s.InstanceName().TODOInstanceName()),
		otherInstances: otherInstances,
	}
	f.appendOp(op)
	return f.maybeErr(op)
...
func (f *fakeSnappyBackend) RunInhibitSnapForUnlink(info *snap.Info, hint runinhibit.Hint, stateUnlocker runinhibit.Unlocker, decision func() error) (lock *osutil.FileLock, err error) {
	...
	lock, err = snaplock.OpenLock(info.InstanceName().TODOInstanceName())
	if err != nil {
		return nil, err
	}
	lock.Lock()
	...
	if err := runinhibit.LockWithHint(info.InstanceName().TODOInstanceName(), hint, inhibitInfo, stateUnlocker); err != nil {
		return nil, err
	}
...
func (f *fakeSnappyBackend) InitXDGDirs(info *snap.Info) error {
	op := &fakeOp{op: "init-xdg-dirs", name: info.InstanceName().TODOInstanceName()}
	f.appendOp(op)
	return f.maybeErr(op)
}
