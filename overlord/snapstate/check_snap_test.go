...
version: 1.0`
	...
	checkCb := func(st *state.State, s, cur *snap.Info, sf snap.Container, flags snapstate.Flags, deviceCtx snapstate.DeviceContext) error {
		...
		c.Assert(s.InstanceName().TODOInstanceName(), Equals, "foo")
...
version: 1.0`
	...
	checkCb := func(st *state.State, s, cur *snap.Info, sf snap.Container, flags snapstate.Flags, deviceCtx snapstate.DeviceContext) error {
		c.Assert(sf, NotNil)
		c.Assert(s.InstanceName().TODOInstanceName(), Equals, "foo_instance")
		c.Assert(s.SnapName().TODOSnapName(), Equals, "foo")
		c.Assert(s.SnapID, Equals, "snap-id")
		checkCbCalled = true
		return nil
	}
...
func (s *checkSnapSuite) testCheckSnapSystemUsernamesCallsCommon(c *C, expectedUser, expectedID, yaml string) {
	...
	for _, classic := range []bool{false, true} {
		...
		err = snapstate.CheckSnap(s.st, "snap-path", info.SnapName().TODOSnapName(), nil, nil, snapstate.Flags{}, nil)
		c.Assert(err, IsNil)
		if classic {
