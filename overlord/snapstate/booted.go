...
func UpdateBootRevisions(st *state.State) error {
	...
	for _, typ := range []snap.Type{snap.TypeKernel, snap.TypeBase} {
		...
		info, err := CurrentInfo(st, actual.SnapName().TODOSnapName())
		if err != nil {
		...
		if actual.SnapRevision() != info.SideInfo.Revision {
			...
			installTS, err := revertToRevisionTaskSet(st, actual.SnapName().TODOSnapName(), actual.SnapRevision(), Flags{}, "", noRestartBoundaries)
			if err != nil {
				return err
			}
