...
func WriteConfdbFromSnap(hookCtx *hookstate.Context, view *confdb.View, values map[string]any, opts *client.ConfdbOptions) (err error) {
	...
	ts, commitTask, clearTxTask, err := createChangeConfdbTasks(st, tx, view, hookCtx.InstanceName().TODOInstanceName())
	if err != nil {
		return err
	}
...
func getCustodianPlugsForView(st *state.State, view *confdb.View) ([]string, map[string]*snap.PlugInfo, error) {
	...
	for _, plug := range plugs {
		conns, err := repo.Connected(plug.Snap.InstanceName().TODOInstanceName(), plug.Name)
		if err != nil {
			return nil, nil, err
		}
		if len(conns) == 0 {
		...
		custodianPlugs[plug.Snap.InstanceName().TODOInstanceName()] = plug
		custodians = append(custodians, plug.Snap.InstanceName().TODOInstanceName())
	}
...
func getPlugsAffectedByPaths(st *state.State, dbSchema *confdb.Schema, storagePaths [][]confdb.Accessor) (map[string][]*snap.PlugInfo, error) {
	...
	for _, plug := range plugs {
		...
		snapPlugs := affectedPlugs[plug.Snap.InstanceName().TODOInstanceName()]
		affectedPlugs[plug.Snap.InstanceName().TODOInstanceName()] = append(snapPlugs, plug)
	}
