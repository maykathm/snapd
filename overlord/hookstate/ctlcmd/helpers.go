...
func runServiceCommand(context *hookstate.Context, inst *servicestate.Instruction) error {
	...
	var err error
	inst.Names, _, err = maybePatchServiceNames(context.InstanceName().TODOInstanceName(), inst.Names)
	if err != nil {
		return err
	}

	st := context.State()
	appInfos, err := getServiceInfos(st, context.InstanceName().TODOInstanceName(), inst.Names)
	if err != nil {
		return err
	}
...
func createSnapctlInstallTasks(hctx *hookstate.Context, cmd managementCommand) (affectedComponents []string, tss []*state.TaskSet, err error) {
	...
	var snapst snapstate.SnapState
	if err := snapstate.Get(st, instanceName.TODOInstanceName(), &snapst); err != nil {
		return nil, nil, err
	}
	...
	snapName := snap.InstanceSnap(instanceName.TODOInstanceName())
	if vsets != nil {
	...
	if len(affectedComponents) == 0 {
		return nil, nil, snap.NewAlreadyInstalledComponentsError(instanceName.TODOInstanceName(), cmd.components)
	}
...
func createSnapctlRemoveTasks(hctx *hookstate.Context, cmd managementCommand) (tss []*state.TaskSet, err error) {
	...
	return snapstateRemoveComponents(st, hctx.InstanceName().TODOInstanceName(), cmd.components,
