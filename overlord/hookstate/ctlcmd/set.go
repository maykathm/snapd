...
func (s *setCommand) Execute(args []string) error {
	...
	if s.View {
		...
		return setConfdbValues(context, name, requests, getVisibilitiesFromCommand(&s.baseCommand))
	}
...
func setConfdbValues(ctx *hookstate.Context, plugName string, requests map[string]any, visibilities []confdb.Visibility) error {
	ctx.Lock()
	defer ctx.Unlock()
	...
	err = confdbstate.SetViaView(tx, view, requests, visibilities)
	if err != nil {
		return err
	}
