...
func (s *setCommand) setConfigSetting(context *hookstate.Context) error {
	...
	for _, key := range confKeys {
		if err := tr.Set(s.context().InstanceName().TODOInstanceName(), key, confValues[key]); err != nil {
			return err
		}
	}
...
func setInterfaceAttribute(context *hookstate.Context, staticAttrs map[string]any, dynamicAttrs map[string]any, key string, value any) error {
	...
	var existing any
	err = getAttribute(context.InstanceName().TODOInstanceName(), subkeys[:1], 0, staticAttrs, &existing)
	if err == nil {
	...
	_, err = config.PatchConfig(context.InstanceName().TODOInstanceName(), subkeys, 0, dynamicAttrs, &raw)
	return err
}
