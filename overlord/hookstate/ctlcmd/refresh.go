...
func getUpdateDetails(context *hookstate.Context) (*updateDetails, error) {
	...
	if affectedInfo, ok := affected[context.InstanceName().TODOInstanceName()]; ok {
		base = affectedInfo.Base
		restart = affectedInfo.Restart
	}

	var snapst snapstate.SnapState
	if err := snapstate.Get(st, context.InstanceName().TODOInstanceName(), &snapst); err != nil {
	...
	hasRefreshControl, err := hasSnapRefreshControlInterface(st, context.InstanceName().TODOInstanceName())
	if err != nil {
		return nil, err
	}
...
func (c *refreshCommand) printTrackingInfo(context *hookstate.Context) error {
	...
	st := context.State()
	var snapst snapstate.SnapState
	err := snapstate.Get(st, context.InstanceName().TODOInstanceName(), &snapst)
	if err != nil {
...
func (c *refreshCommand) hold() error {
	...
	affecting, err := snapstate.AffectingSnapsForAffectedByRefreshCandidates(st, ctx.InstanceName().TODOInstanceName())
	if err != nil {
		return err
	}
	if len(affecting) == 0 {
	...
	remaining, err := snapstate.HoldRefresh(st, snapstate.HoldAutoRefresh, ctx.InstanceName().TODOInstanceName(), holdDuration, affecting...)
	if err != nil {
...
func (c *refreshCommand) proceed() error {
	...
	if ctx.IsEphemeral() {
		st := ctx.State()
		hasRefreshControl, err := hasSnapRefreshControlInterface(st, ctx.InstanceName().TODOInstanceName())
		if err != nil {
			return err
		}
		if !hasRefreshControl {
		...
		return autoRefreshForGatingSnap(st, ctx.InstanceName().TODOInstanceName())
	}
...
func (c *refreshCommand) printInhibitLockHint() error {
	...
	hint, _, err := runinhibit.IsLocked(snapName.TODOInstanceName(), nil)
	if err != nil {
		return err
	}
	c.printf("%s", hint)
