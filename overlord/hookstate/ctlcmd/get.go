...
func (c *getCommand) getConfigSetting(context *hookstate.Context) error {
	...
	return c.printValues(func(key string) (any, bool, error) {
		var value any
		err := transaction.Get(c.context().InstanceName().TODOInstanceName(), key, &value)
		if err == nil {
...
func (c *getCommand) getInterfaceSetting(context *hookstate.Context, plugOrSlot string) error {
	...
	return c.printValues(func(key string) (any, bool, error) {
		...
		var value any
		err = getAttribute(context.InstanceName().TODOInstanceName(), subkeys, 0, staticAttrs, &value)
		if err == nil {
			return value, true, nil
		}
		if isNoAttribute(err) {
			err = getAttribute(context.InstanceName().TODOInstanceName(), subkeys, 0, dynamicAttrs, &value)
			if err == nil {
				return value, true, nil
			}
		}
...
func checkConfdbPlugConnection(ctx *hookstate.Context, plugName string) (*snap.PlugInfo, error) {
	...
	plug := repo.Plug(ctx.InstanceName().TODOInstanceName(), plugName)
	if plug == nil {
	...
	conns, err := repo.Connected(ctx.InstanceName().TODOInstanceName(), plugName)
	if err != nil {
