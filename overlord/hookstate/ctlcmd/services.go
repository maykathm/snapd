...
func (c *servicesCommand) Execute([]string) error {
	...
	serviceNames, patched, err := maybePatchServiceNames(ctx.InstanceName().TODOInstanceName(), serviceNames)
	if err != nil {
		return err
	}

	st := ctx.State()
	svcInfos, err := getServiceInfos(st, ctx.InstanceName().TODOInstanceName(), serviceNames)
	if err != nil {
		return err
	}
