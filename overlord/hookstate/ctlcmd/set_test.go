...
func (s *confdbSuite) TestConfdbSetSingleView(c *C) {
	...
	val, err := tx.Get(parsePath(c, "wifi.ssid"), nil, confdb.Schema{}, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *confdbSuite) TestConfdbSetManyViews(c *C) {
	...
	val, err = tx.Get(parsePath(c, "wifi.psk"), nil, confdb.Schema{}, []confdb.Visibility{})
	c.Assert(err, IsNil)
...
func (s *confdbSuite) TestConfdbSetExclamationMark(c *C) {
	...
	_, err = tx.Get(parsePath(c, "wifi.psk"), nil, confdb.Schema{}, []confdb.Visibility{})
