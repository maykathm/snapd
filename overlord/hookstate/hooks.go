...
func (h *gateAutoRefreshHookHandler) Before() error {
	...
	snapInfo, err := snapstate.CurrentInfo(st, snapName.TODOInstanceName())
	if err != nil {
		return err
	}
	...
	lock, err := snaplock.OpenLock(snapName.TODOInstanceName())
	if err != nil {
		return err
	}
	...
	if err := runinhibit.LockWithHint(snapName.TODOInstanceName(), runinhibit.HintInhibitedGateRefresh, inhibitInfo, st.Unlocker()); err != nil {
		return err
	}
...
func (h *gateAutoRefreshHookHandler) Done() (err error) {
	...
	if a == nil {
		...
		// refresh inhibit).
		if err := runinhibit.Unlock(snapName.TODOInstanceName(), st.Unlocker()); err != nil {
			return fmt.Errorf("cannot unlock inhibit lock for snap %s: %v", snapName, err)
		}
		return snapstate.ProceedWithRefresh(st, snapName.TODOInstanceName(), nil)
	} else {
		var ok bool
	...
	case snapstate.GateAutoRefreshProceed:
		...
		if err := snapstate.ProceedWithRefresh(st, snapName.TODOInstanceName(), nil); err != nil {
			return err
		}
		...
		_, inhibitInfo, err := runinhibit.IsLocked(snapName.TODOInstanceName(), nil)
		if err != nil {
			return err
		}
		if err := runinhibit.LockWithHint(snapName.TODOInstanceName(), runinhibit.HintInhibitedForRefresh, inhibitInfo, nil); err != nil {
...
func (h *gateAutoRefreshHookHandler) Error(hookErr error) (ignoreHookErr bool, err error) {
	...
	affecting, err := snapstate.AffectingSnapsForAffectedByRefreshCandidates(st, snapName.TODOInstanceName())
	if err != nil {
	...
	if _, err := snapstate.HoldRefresh(st, snapstate.HoldAutoRefresh, snapName.TODOInstanceName(), holdDuration, affecting...); err != nil {
