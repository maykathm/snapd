...
func (s *interfaceManagerSuite) TestConnectTask(c *C) {
	...
	c.Assert(plug.Snap.TODOInstanceName(), Equals, "consumer")
	c.Assert(plug.Name, Equals, "plug")
	var slot interfaces.SlotRef
	c.Assert(task.Get("slot", &slot), IsNil)
	c.Assert(slot.Snap.TODOInstanceName(), Equals, "producer")
...
func (s *interfaceManagerSuite) TestParallelInstallConnectTask(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(plug.Snap.TODOInstanceName(), Equals, "consumer_foo")
	c.Assert(plug.Name, Equals, "plug")
	var slot interfaces.SlotRef
...
func (s *interfaceManagerSuite) getConnection(c *C, plugSnap, plugName, slotSnap, slotName string) *interfaces.Connection {
	conn, err := s.manager(c).Repository().Connection(&interfaces.ConnRef{
		PlugRef: interfaces.PlugRef{Snap: naming.InstanceName(plugSnap), Name: plugName},
		SlotRef: interfaces.SlotRef{Snap: naming.InstanceName(slotSnap), Name: slotName},
	})
	c.Assert(err, IsNil)
...
func (s *interfaceManagerSuite) mockSnapInstance(c *C, instanceName, yamlText string) *snap.Info {
	...
	snapInfo.RealName = snapInfo.SnapName().TODOSnapName()
	...
	snapstate.Set(s.state, snapInfo.InstanceName().TODOInstanceName(), &snapstate.SnapState{
		Active:      true,
		Sequence:    snapstatetest.NewSequenceFromSnapSideInfos([]*snap.SideInfo{sideInfo}),
		Current:     sideInfo.Revision,
...
func (s *interfaceManagerSuite) mockUpdatedSnap(c *C, yamlText string, revision int) *snap.Info {
	...
	var snapst snapstate.SnapState
	err := snapstate.Get(s.state, snapInfo.InstanceName().TODOInstanceName(), &snapst)
	c.Assert(err, IsNil)
	snapst.Sequence.Revisions = append(snapst.Sequence.Revisions, sequence.NewRevisionSideState(sideInfo, nil))
	snapstate.Set(s.state, snapInfo.InstanceName().TODOInstanceName(), &snapst)
...
func (s *interfaceManagerSuite) TestDoSetupSnapSecurityHonorsUndesiredFlag(c *C) {
	...
	change := s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: snapInfo.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) testDoSetupSnapSecurityNoAutoConnectParallelInstalledSlotSnap(c *C, scenario int) {
	...
	case installingConsumer:
		...
		change = s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
			SideInfo: &snap.SideInfo{
				RealName: consSnapInfo.SnapName().TODOSnapName(),
	...
	case installingProducer:
		...
		change = s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
			SideInfo: &snap.SideInfo{
				RealName: prodSnapInfo.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) TestDoSetupSnapSecurityAutoConnectsSlotsMultiplePlugs(c *C) {
	...
	change := s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: producer.Info().SnapName().TODOSnapName(),
...
`))
	...
	producerSnapSetup := &snapstate.SnapSetup{SideInfo: &snap.SideInfo{RealName: producerInfo.SnapName().TODOSnapName(), SnapID: producerInfo.SnapID, Revision: producerInfo.Revision}}
	consumerSnapSetup := &snapstate.SnapSetup{SideInfo: &snap.SideInfo{RealName: consumerInfo.SnapName().TODOSnapName(), SnapID: consumerInfo.SnapID, Revision: consumerInfo.Revision}}
	...
	for _, call := range secBackend.SetupManyCalls[1:4] {
		...
		singleSnapCalls = append(singleSnapCalls, call.AppSets[0].InstanceName().TODOInstanceName())
	}
...
func (s *interfaceManagerSuite) TestDoSetupProfilesAddsImplicitSlots(c *C) {
	...
	repo := mgr.Repository()
	slots := repo.Slots(snapInfo.InstanceName().TODOInstanceName())
...
func (s *interfaceManagerSuite) TestDoSetupSnapSecurityReloadsConnectionsWhenInvokedOnPlugSide(c *C) {
	...
	snapInfo := s.mockSnap(c, consumerYaml)
	s.mockSnap(c, producerYaml)
	s.testDoSetupSnapSecurityReloadsConnectionsWhenInvokedOn(c, snapInfo.InstanceName().TODOInstanceName(), snapInfo.Revision)
...
func (s *interfaceManagerSuite) TestSetupProfilesUsesFreshSnapInfo(c *C) {
	...
	change := s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: newSnapInfo.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) TestSetupProfilesOnInstall(c *C) {
	...
	change := s.addSetupSnapSecurityChangeWithOptions(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: installSnapInfo.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) mockComponentForSnap(c *C, compName string, compYaml string, snapInfo *snap.Info) *snap.ComponentInfo {
	...
	var snapst snapstate.SnapState
	c.Assert(snapstate.Get(s.state, snapInfo.InstanceName().TODOInstanceName(), &snapst), IsNil)

	snapst.Sequence.AddComponentForRevision(snapInfo.Revision, &sequence.ComponentState{
		SideInfo: &snap.ComponentSideInfo{
			Component: naming.NewComponentRef(snapInfo.SnapName().TODOSnapName(), compName),
			Revision:  snap.R(1),
		},
...
func (s *interfaceManagerSuite) TestSetupProfilesOfAffectedSnapWithComponents(c *C) {
	...
	snapst.Sequence.AddComponentForRevision(snapInfo.Revision, &sequence.ComponentState{
		SideInfo: &snap.ComponentSideInfo{
			Component: naming.NewComponentRef(snapInfo.SnapName().TODOSnapName(), "comp2"),
			Revision:  snap.R(1),
		},
		CompType: snap.StandardComponent,
	})
	...
	c.Assert(snapstate.Get(s.state, coreSnapInfo.InstanceName().TODOInstanceName(), &coreSnapst), IsNil)
	coreSnapst.Sequence.AddComponentForRevision(snapInfo.Revision, &sequence.ComponentState{
		SideInfo: &snap.ComponentSideInfo{
			Component: naming.NewComponentRef(snapInfo.SnapName().TODOSnapName(), "comp"),
			Revision:  snap.R(1),
		},
		CompType: snap.StandardComponent,
	})
	...
	snapstate.Set(s.state, snapInfo.InstanceName().TODOInstanceName(), &snapst)
	snapstate.Set(s.state, coreSnapInfo.InstanceName().TODOInstanceName(), &coreSnapst)
...
func (s *interfaceManagerSuite) testAutoconnectionsRemovedForMissingSlots(c *C, expectedConns map[string]any) {
	...
	change := s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: newSnapInfo1.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) TestAutoConnectSetupSecurityOnceWithMultiplePlugs(c *C) {
	...
	for _, sc := range s.secBackend.SetupCalls {
		setupCalls[sc.AppSet.InstanceName().TODOInstanceName()]++
	}
...
func (s *interfaceManagerSuite) TestSetupProfilesDevModeMultiple(c *C) {
	...
	change := s.addSetupSnapSecurityChange(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: siC.Info().SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) TestUndoSetupProfilesOnInstall(c *C) {
	...
	c.Check(s.secBackend.RemoveCalls, DeepEquals, []string{snapInfo.InstanceName().TODOInstanceName()})
...
`
	...
	s.state.Lock()
	var snapst snapstate.SnapState
	c.Assert(snapstate.Get(s.state, producerInfo.InstanceName().TODOInstanceName(), &snapst), IsNil)
	snapst.Active = false
	snapstate.Set(s.state, producerInfo.InstanceName().TODOInstanceName(), &snapst)

	change := s.state.NewChange("test", "")
	task := s.state.NewTask("setup-profiles", "")
	task.Set("prepare-profiles", true)
	task.Set("snap-setup", &snapstate.SnapSetup{SideInfo: &snap.SideInfo{
		RealName: updatedInfo.SnapName().TODOSnapName(),
...
`, 2)
	...
	change := s.addSetupSnapSecurityChangeWithOptions(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: newConsumerInfo.SnapName().TODOSnapName(),
...
func (s *interfaceManagerSuite) TestUndoSetupProfilesOnRefreshClassicToStrictUsesOldClassicFlag(c *C) {
	...
	s.state.Lock()
	var snapst snapstate.SnapState
	c.Assert(snapstate.Get(s.state, oldSnapInfo.InstanceName().TODOInstanceName(), &snapst), IsNil)
	snapst.Flags.Classic = true
	// Make the snap inactive so prepare-profiles will record PendingSecurity
	// for the refresh attempt.
	snapst.Active = false
	snapstate.Set(s.state, oldSnapInfo.InstanceName().TODOInstanceName(), &snapst)
	s.state.Unlock()
	...
	change := s.addSetupSnapSecurityChangeWithOptions(c, &snapstate.SnapSetup{
		SideInfo: &snap.SideInfo{
			RealName: oldSnapInfo.SnapName().TODOSnapName(),
...
func makeAutoConnectChange(st *state.State, plugSnap, plug, slotSnap, slot string, delayedSetupProfiles bool) *state.Change {
	...
	t.Set("slot", interfaces.SlotRef{Snap: naming.InstanceName(slotSnap), Name: slot})
	t.Set("plug", interfaces.PlugRef{Snap: naming.InstanceName(plugSnap), Name: plug})
...
func (s *interfaceManagerSuite) TestSnapsWithSecurityProfiles(c *C) {
	...
	for _, set := range appSets {
		got[set.InstanceName().TODOInstanceName()] = set.Info().Revision
	}
...
func (s *interfaceManagerSuite) autoconnectChangeForPreseeding(c *C, skipMarkPreseeded bool) (autoconnectTask, markPreseededTask *state.Task) {
	...
	hsup := &hookstate.HookSetup{
		Snap: snapInfo.InstanceName().TODOInstanceName(),
		Hook: "install",
	}
...
func (s *interfaceManagerSuite) TestDelayedEffectsSetupProfilesRunThroughMultipleConnected(c *C) {
	...
	secBackend := &ifacetest.TestSecurityBackendDelayedEffects{
		...
		ApplyDelayedEffectsCallback: func(appSet *interfaces.SnapAppSet, effs []interfaces.DelayedSideEffect) error {
			c.Check([]string{"consumer1", "consumer2"}, testutil.Contains, appSet.InstanceName().TODOInstanceName())
