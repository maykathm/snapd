...
func (m *InterfaceManager) refreshAppSetConnections(task *state.Task, appSet *interfaces.SnapAppSet) ([]string, []string, error) {
	...
	disconnectedSnaps, err := m.repo.DisconnectSnap(instanceName.String())
	if err != nil {
		return nil, nil, err
	}
	...
	if err := m.repo.RemoveSnap(instanceName.String()); err != nil {
		return nil, nil, err
	}
...
func (m *InterfaceManager) setupProfilesForAppSet(
	...
	affectedNames = append([]string{instanceName}, affectedNames...)
