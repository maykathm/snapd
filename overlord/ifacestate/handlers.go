...
func (m *InterfaceManager) setupProfilesForAppSet(
	...
	for _, connID := range reloadedConns {
		...
		affectedSet[connRef.PlugRef.Snap.String()] = true
		affectedSet[connRef.SlotRef.Snap.String()] = true

		// Snaps on the plug or slot side, other than the current one, are
		// indirectly affected.
		if connRef.PlugRef.Snap != instanceName {
			snapsWithConnectedPlugs[connRef.PlugRef.Snap.String()] = true
		}
		if connRef.SlotRef.Snap != instanceName {
			snapsWithConnectedSlots[connRef.SlotRef.Snap.String()] = true
		}
	}
	...
	for _, connId := range newConns {
		...
		newConnectedSnaps[connRef.PlugRef.Snap.String()] = true
		newConnectedSnaps[connRef.SlotRef.Snap.String()] = true
	}
...
func (m *InterfaceManager) doConnect(task *state.Task, _ *tomb.Tomb) (err error) {
	...
	plug := m.repo.Plug(connRef.PlugRef.Snap.String(), connRef.PlugRef.Name)
	if plug == nil {
		// conflict logic should prevent this
	...
	slot := m.repo.Slot(connRef.SlotRef.Snap.String(), connRef.SlotRef.Name)
	if slot == nil {
		// conflict logic should prevent this
	...
	defer func() {
		if err != nil {
			if err := m.repo.Disconnect(plugRef.Snap.String(), plugRef.Name, slotRef.Snap.String(), slotRef.Name); err != nil {
...
func (m *InterfaceManager) doDisconnect(task *state.Task, _ *tomb.Tomb) error {
	...
	err = m.repo.Disconnect(plugRef.Snap.String(), plugRef.Name, slotRef.Snap.String(), slotRef.Name)
	if err != nil {
...
func (m *InterfaceManager) undoConnect(task *state.Task, _ *tomb.Tomb) error {
	...
	if err := m.repo.Disconnect(connRef.PlugRef.Snap.String(), connRef.PlugRef.Name, connRef.SlotRef.Snap.String(), connRef.SlotRef.Name); err != nil {
		return err
	}
...
func (m *InterfaceManager) transitionConnectionsCoreMigration(st *state.State, oldName, newName string) error {
	...
	for id := range conns {
		...
		if connRef.SlotRef.Snap.String() == oldName {
...
func (m *InterfaceManager) doHotplugConnect(task *state.Task, _ *tomb.Tomb) error {
	...
	for _, id := range connsForDevice {
		...
		if err := checkAutoconnectConflicts(st, task, connRef.PlugRef.Snap.String(), connRef.SlotRef.Snap.String()); err != nil {
	...
	for _, conn := range recreate {
		...
		ts, err := connect(st, conn.PlugRef.Snap.String(), conn.PlugRef.Name, conn.SlotRef.Snap.String(), conn.SlotRef.Name, connectOpts{AutoConnect: wasAutoconnected})
		if err != nil {
	...
	for _, conn := range newconns {
		ts, err := connect(st, conn.PlugRef.Snap.String(), conn.PlugRef.Name, conn.SlotRef.Snap.String(), conn.SlotRef.Name, connectOpts{AutoConnect: true})
		if err != nil {
