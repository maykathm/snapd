...
ConnsLoop:
	for connId, connState := range conns {
		...
		if snapName != "" && connRef.PlugRef.Snap.String() != snapName && connRef.SlotRef.Snap.String() != snapName {
			continue
		}

		plugInfo := m.repo.Plug(connRef.PlugRef.Snap.String(), connRef.PlugRef.Name)
		slotInfo := m.repo.Slot(connRef.SlotRef.Snap.String(), connRef.SlotRef.Name)
		...
		if plugInfo == nil || slotInfo == nil {
			...
			if connState.Auto && !connState.ByGadget && connState.Interface != "core-support" {
				...
				for _, snapName := range []string{connRef.PlugRef.Snap.String(), connRef.SlotRef.Snap.String()} {
...
func (m *InterfaceManager) removeConnections(snapName string) error {
	...
	for id := range conns {
		...
		if connRef.PlugRef.Snap.String() == snapName || connRef.SlotRef.Snap.String() == snapName {
	...
	for _, conn := range repoConns {
		if err := m.repo.Disconnect(conn.PlugRef.Snap.String(), conn.PlugRef.Name, conn.SlotRef.Snap.String(), conn.SlotRef.Name); err != nil {
			return fmt.Errorf("internal error: %v", err)
		}
	}
	return nil
