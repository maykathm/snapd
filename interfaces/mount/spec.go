...
func (spec *Specification) AddOvername(info *snap.Info) {
	...
	spec.AddOvernameMountEntry(osutil.MountEntry{
		Name:    path.Join(dirs.CoreSnapMountDir, info.InstanceName().TODOInstanceName()),
		Dir:     path.Join(dirs.CoreSnapMountDir, info.SnapName().TODOSnapName()),
		Options: []string{"rbind", osutil.XSnapdOriginOvername()},
	})
	// /var/snap/foo_bar -> /var/snap/foo
	spec.AddOvernameMountEntry(osutil.MountEntry{
		Name:    path.Join(dirs.SnapDataDir, info.InstanceName().TODOInstanceName()),
		Dir:     path.Join(dirs.SnapDataDir, info.SnapName().TODOSnapName()),
		Options: []string{"rbind", osutil.XSnapdOriginOvername()},
	})
}
