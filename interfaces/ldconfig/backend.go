...
func (b *Backend) setupLdconfigCache(spec *Specification) error {
	...
	if len(spec.libDirs) > 0 {
		...
		sort.Slice(sortedSnapSlots, func(i, j int) bool {
			n1 := string(sortedSnapSlots[i].snapSlot.SnapName) + "_" + sortedSnapSlots[i].snapSlot.SlotName
			n2 := string(sortedSnapSlots[j].snapSlot.SnapName) + "_" + sortedSnapSlots[j].snapSlot.SlotName
			return n1 < n2
		})
