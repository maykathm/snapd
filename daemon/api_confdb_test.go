...
func (s *confdbSuite) TestGetView(c *C) {
	...
	for _, t := range []test{
		...
		restoreLoad := daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, view *confdb.View, requests []string, _ map[string]any, _ []confdb.Visibility) (string, error) {
...
func (s *confdbSuite) TestViewGetMany(c *C) {
	...
	restore = daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, view *confdb.View, requests []string, _ map[string]any, _ []confdb.Visibility) (string, error) {
...
func (s *confdbSuite) TestViewSetMany(c *C) {
	...
	restore = daemon.MockConfdbstateSetViaView(func(_ confdb.Databag, view *confdb.View, values map[string]any, _ []confdb.Visibility) error {
		c.Assert(view.Name, Equals, "wifi-setup")
...
func (s *confdbSuite) TestGetErrorHandling(c *C) {
	s.setFeatureFlag(c)
	...
	for _, t := range []test{
		...
		restore := daemon.MockConfdbstateLoadConfdbAsync(func(*state.State, *confdb.View, []string, map[string]any, []confdb.Visibility) (string, error) {
			return "", t.err
		})
...
func (s *confdbSuite) TestGetViewMisshapenQuery(c *C) {
	...
	restore = daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, _ *confdb.View, requests []string, _ map[string]any, _ []confdb.Visibility) (string, error) {
...
func (s *confdbSuite) TestSetView(c *C) {
	...
	for _, t := range []test{
		...
		var called bool
		restoreSet := daemon.MockConfdbstateSetViaView(func(bag confdb.Databag, view *confdb.View, values map[string]any, _ []confdb.Visibility) error {
			called = true
			c.Assert(view.Name, Equals, "wifi-setup")
...
func (s *confdbSuite) TestSetEmpty(c *C) {
	...
	var called bool
	restore = daemon.MockConfdbstateSetViaView(func(bag confdb.Databag, view *confdb.View, values map[string]any, _ []confdb.Visibility) error {
		called = true
		return nil
	})
	defer restore()
...
func (s *confdbSuite) TestGetConstraints(c *C) {
	...
	restore = daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, _ *confdb.View, requests []string, constraints map[string]any, _ []confdb.Visibility) (string, error) {
		c.Assert(requests, DeepEquals, []string{"ssid"})
...
func (s *confdbSuite) TestGetBadConstraints(c *C) {
	...
	restore = daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, _ *confdb.View, _ []string, _ map[string]any, _ []confdb.Visibility) (string, error) {
...
func (s *confdbSuite) TestGetViewCheckVisibility(c *C) {
	...
	type test struct {
		name         string
		pid          string
		visibilities []confdb.Visibility
	}
	restoreGet := daemon.MockConfdbstateGetView(func(_ *state.State, acc, confdbSchema, viewName string) (*confdb.View, error) {
		return s.schema.View(viewName), nil
	})
	defer restoreGet()
	for _, t := range []test{
		{name: "non-root", pid: "1000", visibilities: []confdb.Visibility{confdb.SecretVisibility}},
		{name: "root", pid: "0", visibilities: []confdb.Visibility{}},
	} {
		cmt := Commentf("%s test", t.name)
		restoreLoad := daemon.MockConfdbstateLoadConfdbAsync(func(_ *state.State, _ *confdb.View, _ []string, _ map[string]any, v []confdb.Visibility) (string, error) {
			c.Assert(v, DeepEquals, t.visibilities)
			return "123", nil
		})
		req, err := http.NewRequest("GET", "/v2/confdb/system/network/wifi-setup?keys=ssid", nil)
		c.Assert(err, IsNil, cmt)
...
func (s *confdbSuite) TestSetViewCheckVisibility(c *C) {
	s.setFeatureFlag(c)
	...
	for _, t := range []test{
		...
		restoreSet := daemon.MockConfdbstateSetViaView(func(bag confdb.Databag, view *confdb.View, values map[string]any, v []confdb.Visibility) error {
			c.Assert(v, DeepEquals, t.visibilities)
			return nil
		})
