...
func (s *snapsSuite) testRevertSnap(inst *daemon.SnapInstruction, c *check.C) {
	...
	defer daemon.MockSnapstateRevert(func(s *state.State, name naming.InstanceName, flags snapstate.Flags, fromChange string) (*state.TaskSet, error) {
		c.Check(flags, check.Equals, instFlags)
		queue = append(queue, name.String())
		return nil, nil
	})()
	defer daemon.MockSnapstateRevertToRevision(func(s *state.State, name naming.InstanceName, rev snap.Revision, flags snapstate.Flags, fromChange string) (*state.TaskSet, error) {
		c.Check(flags, check.Equals, instFlags)
