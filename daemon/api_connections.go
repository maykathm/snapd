...
func (c *collectFilter) plugOrConnectedSlotMatches(plug *interfaces.PlugRef, connectedSlots []interfaces.SlotRef) bool {
	...
	if c.snapName != "" && plug.Snap.TODOInstanceName() != c.snapName {
		return false
	}
	return true
}
...
func (c *collectFilter) slotOrConnectedPlugMatches(slot *interfaces.SlotRef, connectedPlugs []interfaces.PlugRef) bool {
	...
	if c.snapName != "" && slot.Snap.TODOInstanceName() != c.snapName {
		return false
	}
	return true
}
...
func collectConnections(ifaceMgr *ifacestate.InterfaceManager, filter collectFilter) (*connectionsJSON, error) {
	...
	for crefStr, cstate := range connStates {
		...
		if repo.Plug(cref.PlugRef.Snap.TODOInstanceName(), cref.PlugRef.Name) == nil || repo.Slot(cref.SlotRef.Snap.TODOInstanceName(), cref.SlotRef.Name) == nil {
			continue
		}
	...
	for _, plug := range ifaces.Plugs {
		...
		pj := &plugJSON{
			Snap:        plugRef.Snap.TODOInstanceName(),
			Name:        plugRef.Name,
	...
	for _, slot := range ifaces.Slots {
		...
		sj := &slotJSON{
			Snap:        slotRef.Snap.TODOInstanceName(),
			Name:        slotRef.Name,
