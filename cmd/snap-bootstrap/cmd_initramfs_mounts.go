...
func readSnapInfo(sysSnaps map[snap.Type]*seed.Snap, snapType snap.Type) (*snap.Info, error) {
	...
	info, err := snap.ReadInfoFromMountPoint(seedSnap.SnapName().TODOSnapName(), mountPoint, seedSnap.Path, seedSnap.SideInfo)
	if err != nil {
		return nil, err
	}
...
func doInstall(mst *initramfsMountsState, model *asserts.Model, sysSnaps map[snap.Type]*seed.Snap) error {
	...
	hasDriversTree, err := createKernelMounts(
		rootfsDir, kernelSnap.SnapName().TODOSnapName(), kernelSnap.Revision, !isCore)
	if err != nil {
		return err
	}
...
func generateMountsModeInstall(mst *initramfsMountsState) error {
	...
	if installAndRun {
		...
		// available early.
		kernSnap, err = theSeed.ModeSnap(kernSnap.SnapName().TODOSnapName(), "run")
		if err != nil {
			return err
		}
...
func generateMountsModeRun(mst *initramfsMountsState) error {
	...
	hasDriversTree, err := createKernelMounts(
		rootfsDir, kernPlaceInfo.SnapName().TODOSnapName(), kernPlaceInfo.SnapRevision(), isClassic)
	if err != nil {
		return err
	}
