...
// Package snap_exec implements the snap-exec entry point. snap-exec is called
// by snap-confine after setting up the snap's mount namespace, to exec the
// actual snap application or hook.
package snap_exec
...
func execHook(snapTarget string, revision, hookName string) error {
	...
	if componentName == "" {
		...
		mountDir = snap.ComponentMountDir(component.Component.ComponentName, component.Revision, naming.InstanceName(info.InstanceName()))
	}
