...
func createUserDataDirs(info *snap.Info, opts *dirs.SnapDirOptions) error {
	...
	if info.InstanceKey != "" {
		...
		snapUserDir := snap.UserSnapDir(usr.HomeDir, info.SnapName().TODOSnapName(), opts)
...
func checkSnapRunInhibitionConflict(app *snap.AppInfo) error {
	...
	if hint == runinhibit.HintInhibitedForRemove {
		return fmt.Errorf(i18n.G("cannot run %q, snap is being removed"), snap.JoinSnapApp(snapName.TODOInstanceName(), app.Name))
	}
	if hint == runinhibit.HintInhibitedForDisable {
		return fmt.Errorf(i18n.G("cannot run %q, snap is disabled"), snap.JoinSnapApp(snapName.TODOInstanceName(), app.Name))
	}
	...
	if osutil.FileExists(runinhibit.HintFile(snapName.TODOInstanceName())) {
...
func (r *runnable) Target() string {
	if r.component != nil {
		return snap.SnapComponentName(r.info.InstanceName().TODOInstanceName(), r.component.Component.ComponentName)
	}

	if r.hook != nil {
		return r.info.InstanceName().TODOInstanceName()
	}
...
func (x *cmdRun) runSnapConfine(info *snap.Info, runner runnable, beforeExec func() error, args []string) error {
	...
	opts, err := getSnapDirOptions(info.InstanceName().TODOInstanceName())
	if err != nil {
	...
	if needsTracking {
		...
		if err = cgroupCreateTransientScopeForTracking(appSecurityTag, opts); err != nil {
			...
			default:
				...
				// setting.
				snapTag := snap.SecurityTag(runner.info.InstanceName().TODOInstanceName())
